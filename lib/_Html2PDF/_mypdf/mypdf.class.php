...
if (! defined('__CLASS_MYPDF__')) {
    ...
    class MyPDF extends FPDF_Protection
    {
        ...
        function setbMargin($v)
        {
            $this->bMargin = $v;
        }

        function setcMargin($v)
        {
            $this->cMargin = $v;
        }
        ...
        {
            if (! is_array($c))
                return sprintf('%.3F ', $c) . ($mode ? 'G' : 'g');
            elseif (count($c) == 3)
                return sprintf('%.3F %.3F %.3F ', $c[0], $c[1], $c[2]) . ($mode ? 'RG' : 'rg');
            elseif (count($c) == 4)
                return sprintf('%.3F %.3F %.3F %.3F ', $c[0], $c[1], $c[2], $c[3]) . ($mode ? 'K' : 'k');
            return null;
        }
    }
}
