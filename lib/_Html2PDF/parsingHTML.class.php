...
class parsingHTML
{
    ...
    function parse()
    {
        ...
        if (count($parents))
            HTML2PDF::makeError(5, __FILE__, __LINE__, $parents);
    ...
    /**
     * preparer le texte une seule fois pour gagner du temps.
     * vient de o_WRITE
     *
     * @param
     *            string texte
     ...
     *
     * @param
     *            &array	tableau de retour des données
     * @return null
     */
     ...
     *
     * @param
     *            string	code HTML à identifier
     * @return array correspondante
     */
    function analiseCode($code)
    {
        ...
        if (isset($param['value']))
            $param['value'] = $this->prepareTxt($param['value']);
        if (isset($param['alt']))
            $param['alt'] = $this->prepareTxt($param['alt']);
        if (isset($param['title']))
            $param['title'] = $this->prepareTxt($param['title']);
        if (isset($param['class']))
            $param['class'] = $this->prepareTxt($param['class']);
        ...
        while (! $end) {
            ...
            if ($row['name'] == 'write') {
                ...
                if ($row['name'] == $detect) {
                    if ($level == 0) {
                        $not = true;
                    } // si on est à la premiere balise : on l'ignore
