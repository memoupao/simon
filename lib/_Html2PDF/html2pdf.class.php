...
if (! defined('__CLASS_HTML2PDF__')) {
    ...
    $HTML2PDF_TABLEAU = array(); // tableau global necessaire à la gestion des tables imbriquées
    global $HTML2PDF_TEXTES;
    $HTML2PDF_TEXTES = array(); // tableau comprennant le fichier de langue
    class HTML2PDF
    {
         ...
         *
         * @param
         *            string		sens portrait ou landscape
         * @param
         *            string		format A4, A5, ...
         * @param
         *            string		langue : fr, en, it...
         * @param
         *            array		marges par defaut, dans l'ordre (left, top, right, bottom)
         * @return null
         */
         ...
         *
         * @param
         *            string	nom de l'etape
         * @param
         *            boolean true=monter d'un niveau, false=descendre d'un niveau, null : ne rien faire
         * @return null
         */
         ...
         *
         * @param
         *            string	nom de la fonte par defaut. si null : Arial pour fonte non spécifiée, et erreur pour fonte non existante
         * @return string de l'ancienne fonte par defaut
         */
         ...
         *
         * @param
         *            int		en mm, marge left
         * @param
         *            int		en mm, marge top
         * @param
         *            int		en mm, marge right. si null, left=right
         * @param
         *            int		en mm, marge bottom. si null, bottom=8
         * @return null
         */
        ...
        function setMargins()
        {
            ...
            $this->pageMarges[floor($this->margeTop * 100)] = array(
                $this->margeLeft,
        ...
        function saveMargin($ml, $mt, $mr)
        {
            ...
            $this->pageMarges = array();
            $this->pageMarges[floor($mt * 100)] = array(
                $ml,
                $this->pdf->getW() - $mr
            );
        }
         ...
         *
         * @param
         *            string nom de la fonte
         * @param
         *            string style de la fonte
         * @param
         *            string fichier de la fonte
         * @return null
         */
         ...
         *
         * @param
         *            float	hauteur de la ligne
         * @param
         *            integer	position reelle courante si saut de ligne pendant l'ecriture d'un texte
         * @return null
         */
         ...
         *
         * @param
         *            mixed	format de la page : A5, A4, array(width, height)
         * @param
         *            string	sens P=portrait ou L=landscape
         * @param
         *            array	tableau des propriétés du fond de la page
         * @param
         *            integer	position reelle courante si saut de ligne pendant l'ecriture d'un texte
         * @return null
         */
        ...
        function setNewPositionForNewLine($curr = null)
        {
            ...
            /*
             * if ( $this->style->value['text-align']!='right' && $this->style->value['text-align']!='center' && $this->style->value['text-align']!='justify' ) { //				$this->pdf->setWordSpacing(0); return null; }
             */
            $sub = null;
            ...
            $this->currentH = $h;
            /*
             * if ($this->style->value['text-align']=='justify' && $e>1) $this->pdf->setWordSpacing(($wMax-$w)/($e-1)); else $this->pdf->setWordSpacing(0);
             */
        }
         ...
         *
         * @param
         *            HTML2PDF	futur sous HTML2PDF passé en référence pour création
         * @param
         *            integer		marge eventuelle de l'objet si simulation d'un TD
         * @return null
         */
        function createSubHTML(&$sub_html, $cellmargin = 0)
        {
            ...
            if ($this->style->value['width']) {
                ...
                // clonage
            $sub_html = new HTML2PDF($this->sens, $this->format, $this->langue, array(
                $this->defaultLeft,
                $this->defaultTop,
                $this->defaultRight,
                $this->defaultBottom
        ...
        /**
         * initialise le sous HTML2PDF.
         * Ne pas utiliser directement. seul la fonction createSubHTML doit l'utiliser
         *
         * @return null
         */
        ...
        function listeGetWidth()
        {
            return '7mm';
        }

        function listeGetPadding()
        {
            return '1mm';
        }
         ...
         *
         * @param
         *            string	type de liste : ul, ol
         * @param
         *            string	style de la liste
         * @return null
         */
        function listeAddLevel($type = 'ul', $style = '', $img = null)
        {
            ...
            $this->defLIST[count($this->defLIST)] = array(
                'style' => $style,
                'nb' => 0,
                'img' => $img
            );
         ...
         *
         * @param
         *            string	code HTML à convertir
         * @param
         *            boolean	afficher en pdf (false) ou en html adapté (true)
         * @return null
         */
         ...
         *
         * @param
         *            string	code HTML à adapter
         * @return string HTML adapté
         */
         ...
         *
         * @param
         *            array	paramètres de l'élément de parsing
         * @return null
         */
        ...
        function c_PAGE($param)
        {
            ...
            if ($this->DEBUG_actif)
                $this->DEBUG_add('PAGE n°' . $this->page, false);
        ...
        function o_PAGE_HEADER($param)
        {
            ...
            for ($this->parse_pos; $this->parse_pos < count($this->parsing->code); $this->parse_pos ++) {
                ...
                $this->subHEADER[] = $todo;
                if (strtolower($todo['name']) == 'page_header_sub' && $todo['close'])
                    break;
            }
        ...
        function o_PAGE_FOOTER($param)
        {
            ...
            for ($this->parse_pos; $this->parse_pos < count($this->parsing->code); $this->parse_pos ++) {
                ...
                $this->subFOOTER[] = $todo;
                if (strtolower($todo['name']) == 'page_footer_sub' && $todo['close'])
                    break;
            }
        ...
        function o_NOBREAK($param)
        {
            ...
            $y = $this->pdf->getY();
            if ($sub->maxY < ($this->pdf->getH() - $this->pdf->gettMargin() - $this->pdf->getbMargin()) && $y + $sub->maxY >= ($this->pdf->getH() - $this->pdf->getbMargin()))
                $this->setNewPage();
            $this->destroySubHTML($sub);
        ...
        function o_DIV($param, $other = 'div')
        {
            if ($this->forOneLine)
                return false;
            if ($this->DEBUG_actif)
                $this->DEBUG_add(strtoupper($other), true);
            ...
            if (in_array($other, array(
                ...
                if (isset($param['moveTop']))
                    $this->style->value['margin']['t'] += $param['moveTop'];
                if (isset($param['moveLeft']))
                    $this->style->value['margin']['l'] += $param['moveLeft'];
                if (isset($param['moveDown']))
                    $this->style->value['margin']['b'] += $param['moveDown'];
            }
            
            $align_object = null;
            if ($this->style->value['margin-auto'])
                $align_object = 'center';
            ...
            if (! $this->style->value['position']) {
                ...
                if (($h < ($this->pdf->getH() - $this->pdf->gettMargin() - $this->pdf->getbMargin())) && ($this->pdf->getY() + $h >= ($this->pdf->getH() - $this->pdf->getbMargin())) && ! $this->isInOverflow)
                    $this->setNewPage();
                ...
                if ($parent_w > $w) {
                    if ($align_object == 'center')
                        $this->pdf->setX($this->pdf->getX() + ($parent_w - $w) * 0.5);
                    else 
                        if ($align_object == 'right')
                            $this->pdf->setX($this->pdf->getX() + $parent_w - $w);
                }
            ...
            $this->drawRectangle($this->style->value['x'], $this->style->value['y'], $this->style->value['width'], $this->style->value['height'], $this->style->value['border'], $this->style->value['padding'], 0, $this->style->value['background']);
            ...
            if ($overflow) {
                ...
                $this->pdf->clippingPathOpen($this->style->value['x'] + $marge['l'], $this->style->value['y'] + $marge['t'], $this->style->value['width'], $this->style->value['height']);
        ...
        function o_BLOCKQUOTE($param)
        {
            return $this->o_DIV($param, 'blockquote');
        }

        function o_LEGEND($param)
        {
        ...
        function c_DIV($param, $other = 'div')
        {
            ...
            if ($block)
                $this->o_BR(array());
            if ($this->DEBUG_actif)
                $this->DEBUG_add(strtoupper($other), false);
            
            return true;
        }

        function c_BLOCKQUOTE($param)
        {
            return $this->c_DIV($param, 'blockquote');
        }

        function c_FIELDSET($param)
        {
            return $this->c_DIV($param, 'fieldset');
        }

        function c_LEGEND($param)
        {
        ...
        function o_BARCODE($param)
        {
            ...
            if ($this->testIsDeprecated && (isset($param['bar_h']) || isset($param['bar_w'])))
                HTML2PDF::makeError(9, __FILE__, __LINE__, array(
                    'BARCODE',
                    'bar_h, bar_w'
                ));
            ...
            if (! isset($lst_barcode[$param['type']]))
                $param['type'] == 'C39';
            ...
            $w = $this->style->value['width'];
            if (! $w)
                $w = $this->style->ConvertToMM('50mm');
            $h = $this->style->value['height'];
            if (! $h)
                $h = $this->style->ConvertToMM('10mm');
	...
	<b>ATTENTION :</b> 
 	...
 	<b>WARNING:</b> 
        ...
        function o_WRITE($param)
        {
            ...
            if (in_array($this->style->value['id_balise'], array(
                'fieldset',
                'legend',
                'div',
                'table',
            ...
            if ($this->style->value['text-transform'] != 'none') {
                if ($this->style->value['text-transform'] == 'capitalize')
                    ...
                    if ($this->style->value['text-transform'] == 'uppercase')
                        $txt = strtoupper($txt);
                    else 
                        if ($this->style->value['text-transform'] == 'lowercase')
                            $txt = strtolower($txt);
            }
            ...
            while ($x + $w > $right && $x < $right && count($words)) {
                ...
                if ($right - $left < $wc)
                    $wc = $right - $left;
                    /*
                 * if ($this->pdf->ws) { $oldSpace = $this->pdf->CurrentFont['cw'][' ']; $this->pdf->CurrentFont['cw'][' ']*=(1.+$this->pdf->ws); $wc = $str[1]; $this->pdf->CurrentFont['cw'][' '] = $oldSpace; }
                 */
                if (strlen($str[0])) {
                ...
                if (count($words)) {
                    ...
                    if ($nb > 1000) {
                        ...
                        foreach ($words as $k => $word)
                            $txt .= ($k ? ' ' : '') . $word[0];
                        HTML2PDF::makeError(2, __FILE__, __LINE__, array(
                            $txt,
                            $right - $left,
                            $w
                        ));
            ...
            if (count($words)) {
                ...
                foreach ($words as $k => $word)
                    $txt .= ($k ? ' ' : '') . $word[0];
                    /*
                 * if ($this->pdf->ws) { $oldSpace = $this->pdf->CurrentFont['cw'][' ']; $this->pdf->CurrentFont['cw'][' ']*=(1.+$this->pdf->ws); $w = $this->pdf->GetStringWidth($txt); $this->pdf->CurrentFont['cw'][' '] = $oldSpace; }
                 */
                $this->pdf->setXY($this->pdf->getX(), $y + $dh + $dy);
        ...
        function Image($src, $sub_li = false)
        {
            ...
            if (($y + $h > $this->pdf->getH() - $this->pdf->getbMargin()) && ! $this->isInOverflow) {
                $this->setNewPage();
                $x = $this->pdf->getX();
                $y = $this->pdf->getY();
            }
            ...
            if ($parent_w > $w && $float != 'left') {
                if ($float == 'right' || $this->style->value['text-align'] == 'li_right')
                    $x = $parent_x + $parent_w - $w - $this->style->value['margin']['r'] - $this->style->value['margin']['l'];
            }
         ...
         *            float	Hauteur
         * @param
         *            array	Tableau de style de définition des borders
         * @param
         *            float	padding - marge intérieur au rectangle => non utile mais on le passe en paramètre
         * @param
         *            float	margin - marge exterieur au rectangle
         * @param
         *            array	Tableau de style de définition du background
         * @return null
         */
        function drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background)
        {
            ...
            if ($in_TL[0] <= 0 || $in_TL[1] <= 0)
                $in_TL = null;
            if ($in_TR[0] <= 0 || $in_TR[1] <= 0)
                $in_TR = null;
            if ($in_BR[0] <= 0 || $in_BR[1] <= 0)
                $in_BR = null;
            if ($in_BL[0] <= 0 || $in_BL[1] <= 0)
                $in_BL = null;
            ...
            if ($background['image']) {
                ...
                $i_position = $background['position'] !== null ? $background['position'] : array(
                    0,
                    0
                );
                $i_repeat = $background['repeat'] !== null ? $background['repeat'] : array(
                    true,
                    true
                );
                ...
                if (count($i_infos) < 2) {
                    ...
                    if ($i_repeat[0])
                        ...
                        if (preg_match('/^([-]?[0-9\.]+)%/isU', $i_position[0], $match))
                            $i_position[0] = $b_x + $match[1] * ($b_w - $i_width) / 100;
                        else
                            $i_position[0] = $b_x + $i_position[0];
                    
                    if ($i_repeat[1])
                        $i_position[1] = $b_y;
                    else 
                        if (preg_match('/^([-]?[0-9\.]+)%/isU', $i_position[1], $match))
                            $i_position[1] = $b_y + $match[1] * ($b_h - $i_height) / 100;
                        else
                            $i_position[1] = $b_y + $i_position[1];
         ...
         *
         * @param
         *            array	liste des points definissant le tour de la ligne
         * @param
         *            float	couleur RVB
         * @param
         *            string	type de ligne
         * @param
         *            float	largeur de la ligne
         * @return null
         */
        ...
        function o_BR($param, $curr = null)
        {
            ...
            if ($this->maxH == 0)
                $this->maxY = max($this->maxY, $this->pdf->getY() + $h);
        ...
        function o_HR($param)
        {
            ...
            $h = $this->style->value['height'];
            if ($h)
                $h -= $this->style->value['border']['t']['width'] + $this->style->value['border']['b']['width'];
            if ($h <= 0)
                $h = $this->style->value['border']['t']['width'] + $this->style->value['border']['b']['width'];
        ...
        function o_ADDRESS($param)
        {
            return $this->o_I($param, 'address');
        }

        function o_CITE($param)
        {
        ...
        function c_ADDRESS($param)
        {
            return $this->c_I($param);
        }

        function c_CITE($param)
        {
            return $this->c_I($param);
        }
        ...
        function o_FONT($param)
        {
            return $this->o_SPAN($param, 'font');
        }

        function o_LABEL($param)
        {
        ...
        function c_FONT($param)
        {
            return $this->c_SPAN($param);
        }

        function c_LABEL($param)
        {
            return $this->c_SPAN($param);
        }
        ...
        function o_P($param)
        {
            ...
            if (! in_array($this->previousCall, array(
                'c_P',
                'c_UL'
            ))) {
                if ($this->maxH)
            ...
            if ($this->style->value['text-indent'] > 0) {
                ...
                $this->pageMarges[floor($y * 100)] = array(
                    $mL + $this->style->value['text-indent'],
                    $this->pdf->getW() - $mR
                );
        ...
        function o_PRE($param, $other = 'pre')
        {
            ...
            if ($other == 'pre')
                return $this->o_DIV($param, $other);
            
            return true;
        }

        function o_CODE($param)
        {
        ...
        function o_THEAD($param)
        {
            ...
            if ($this->sub_part) {
                ...
                for ($pos = $this->temp_pos; $pos < count($this->parsing->code); $pos ++) {
                    ...
                    if (strtolower($todo['name']) == 'thead_sub' && $todo['close'])
                        break;
                }
            } else {
                $level = $this->parsing->getLevel($this->parse_pos);
        ...
        function o_TFOOT($param)
        {
            ...
            if ($this->sub_part) {
                ...
                for ($pos = $this->temp_pos; $pos < count($this->parsing->code); $pos ++) {
                    ...
                    if (strtolower($todo['name']) == 'tfoot_sub' && $todo['close'])
                        break;
                }
            } else {
                $level = $this->parsing->getLevel($this->parse_pos);
        ...
        function o_TABLE($param, $other = 'table')
        {
            ...
            if (isset($param['align']))
                unset($param['align']);
            if (! in_array($align_object, array(
                'left',
                'center',
                'right'
            )))
            ...
            if ($this->sub_part) {
                if ($this->DEBUG_actif)
                    $this->DEBUG_add('Table n°' . $param['num'], true);
                ...
                $this->drawRectangle($HTML2PDF_TABLEAU[$param['num']]['curr_x'], $HTML2PDF_TABLEAU[$param['num']]['curr_y'], $HTML2PDF_TABLEAU[$param['num']]['width'], isset($HTML2PDF_TABLEAU[$param['num']]['height'][0]) ? $HTML2PDF_TABLEAU[$param['num']]['height'][0] : null, $this->style->value['border'], $this->style->value['padding'], 0, $this->style->value['background']);
            ...
            if ($this->sub_part) {
                ...
                if ($parent_w > $w) {
                    if ($HTML2PDF_TABLEAU[$param['num']]['align'] == 'center')
                        ...
                        if ($HTML2PDF_TABLEAU[$param['num']]['align'] == 'right')
                            $x = $x + $parent_w - $w;
                ...
                for ($k = 0; $k < count($HTML2PDF_TABLEAU[$param['num']]['cases']); $k ++) {
                    ...
                    if (in_array($k, $HTML2PDF_TABLEAU[$param['num']]['thead']['tr']))
                        continue;
                    if (in_array($k, $HTML2PDF_TABLEAU[$param['num']]['tfoot']['tr']))
                        continue;
                ...
                if ($this->DEBUG_actif)
                    $this->DEBUG_add('Table n°' . $param['num'], false);
            }
            ...
            if (! $this->sub_part) {
                ...
                if (! $this->isInTfoot && $HTML2PDF_TABLEAU[$param['num']]['td_y'] + $HTML2PDF_TABLEAU[$param['num']]['marge']['b'] + $ty + $hfoot > $this->pdf->getH() - $this->pdf->getbMargin()) {
                    ...
                    if (isset($HTML2PDF_TABLEAU[$param['num']]['height'][$HTML2PDF_TABLEAU[$param['num']]['page']])) {
                        ...
                        // initialisation du style des bordures de la premiere partie de tableau
                        $this->drawRectangle($HTML2PDF_TABLEAU[$param['num']]['curr_x'], $HTML2PDF_TABLEAU[$param['num']]['curr_y'], $HTML2PDF_TABLEAU[$param['num']]['width'], $HTML2PDF_TABLEAU[$param['num']]['height'][$HTML2PDF_TABLEAU[$param['num']]['page']], $this->style->value['border'], $this->style->value['padding'], $HTML2PDF_TABLEAU[$param['num']]['cellspacing'] * 0.5, $this->style->value['background']);
        ...
        function o_TD($param, $other = 'td')
        {
            ...
            if ($collapse) {
                if (! $this->sub_part) {
                    if (($HTML2PDF_TABLEAU[$param['num']]['tr_curr'] > 1 && ! $HTML2PDF_TABLEAU[$param['num']]['new_page']) || (! $this->isInThead && count($HTML2PDF_TABLEAU[$param['num']]['thead']['code']))) {
                ...
                if ($HTML2PDF_TABLEAU[$param['num']]['td_curr'] > 0) {
                    if (! $return)
                        $this->style->value['width'] += $this->style->value['border']['l']['width'];
            ...
            if ($this->sub_part) {
                ...
                for ($j = 0; $j < $rowspan; $j ++) {
                    for ($i = 0; $i < $colspan; $i ++) {
                        $HTML2PDF_TABLEAU[$param['num']]['corr'][$HTML2PDF_TABLEAU[$param['num']]['corr_y'] + $j][$HTML2PDF_TABLEAU[$param['num']]['corr_x'] + $i] = ($i + $j > 0) ? '' : array(
                            $x,
                            $y,
                            $colspan,
                            $rowspan
                ...
                // initialisation du style des bordures de la premiere partie de tableau
                $this->drawRectangle($HTML2PDF_TABLEAU[$param['num']]['td_x'], $HTML2PDF_TABLEAU[$param['num']]['td_y'], $HTML2PDF_TABLEAU[$param['num']]['cases'][$y][$x]['w'], $HTML2PDF_TABLEAU[$param['num']]['cases'][$y][$x]['h'], $this->style->value['border'], $this->style->value['padding'], $HTML2PDF_TABLEAU[$param['num']]['cellspacing'] * 0.5, $this->style->value['background']);
        ...
        function c_TD($param)
        {
            ...
            if ($this->sub_part) {
                if ($this->testTDin1page && $this->sub_html->pdf->getPage() > 1)
                    HTML2PDF::makeError(7, __FILE__, __LINE__);
                ...
                $HTML2PDF_TABLEAU[$param['num']]['cases'][$HTML2PDF_TABLEAU[$param['num']]['tr_curr'] - 1][$HTML2PDF_TABLEAU[$param['num']]['td_curr'] - 1]['w'] = max(array(
                    $w0,
                    $w2
                ));
                $HTML2PDF_TABLEAU[$param['num']]['cases'][$HTML2PDF_TABLEAU[$param['num']]['tr_curr'] - 1][$HTML2PDF_TABLEAU[$param['num']]['td_curr'] - 1]['h'] = max(array(
                    $h0,
                    $h2
                ));
        ...
        {
            /*
             * // construction d'un tableau de correlation $corr = array(); // on fait correspondre chaque case d'un tableau normé aux cases réelles, en prennant en compte les colspan et rowspan $Yr=0; for($y=0; $y<count($cases); $y++) { $Xr=0; 	while(isset($corr[$Yr][$Xr])) $Xr++; for($x=0; $x<count($cases[$y]); $x++) { for($j=0; $j<$cases[$y][$x]['rowspan']; $j++) { for($i=0; $i<$cases[$y][$x]['colspan']; $i++) { $corr[$Yr+$j][$Xr+$i] = ($i+$j>0) ? '' : array($x, $y, $cases[$y][$x]['colspan'], $cases[$y][$x]['rowspan']); } } $Xr+= $cases[$y][$x]['colspan']; while(isset($corr[$Yr][$Xr])) $Xr++; } $Yr++; }
             */
            if (! isset($corr[0]))
                return true;
        ...
        function o_SELECT($param)
        {
            ...
            if (! isset($param['name']))
                $param['name'] = 'champs_pdf_' . (count($this->lstChamps) + 1);
            ...
            if ($this->lstSelect['multi'] && $this->lstSelect['size'] < 3)
                $this->lstSelect['size'] = 3;
        ...
        {
            if (! $this->isInDraw)
                HTML2PDF::makeError(8, __FILE__, __LINE__, 'LINE');
        ...
        {
            if (! $this->isInDraw)
                HTML2PDF::makeError(8, __FILE__, __LINE__, 'RECT');
            $this->pdf->doTransform(isset($param['transform']) ? $this->_prepareTransform($param['transform']) : null);
            $this->style->save();
            $styles = $this->style->getSvgStyle('path', $param);
            $style = $this->pdf->svgSetStyle($styles);
        ...
        {
            if (! $this->isInDraw)
                HTML2PDF::makeError(8, __FILE__, __LINE__, 'CIRCLE');
            $this->pdf->doTransform(isset($param['transform']) ? $this->_prepareTransform($param['transform']) : null);
            $this->style->save();
            $styles = $this->style->getSvgStyle('path', $param);
            $style = $this->pdf->svgSetStyle($styles);
        ...
        {
            if (! $this->isInDraw)
                HTML2PDF::makeError(8, __FILE__, __LINE__, 'ELLIPSE');
            $this->pdf->doTransform(isset($param['transform']) ? $this->_prepareTransform($param['transform']) : null);
            $this->style->save();
            $styles = $this->style->getSvgStyle('path', $param);
            $style = $this->pdf->svgSetStyle($styles);
        ...
        {
            if (! $this->isInDraw)
                HTML2PDF::makeError(8, __FILE__, __LINE__, 'POLYGON');
            $this->pdf->doTransform(isset($param['transform']) ? $this->_prepareTransform($param['transform']) : null);
            $this->style->save();
            $styles = $this->style->getSvgStyle('path', $param);
            $style = $this->pdf->svgSetStyle($styles);
        ...
        {
            if (! $this->isInDraw)
                HTML2PDF::makeError(8, __FILE__, __LINE__, 'PATH');
            $this->pdf->doTransform(isset($param['transform']) ? $this->_prepareTransform($param['transform']) : null);
            $this->style->save();
            $styles = $this->style->getSvgStyle('path', $param);
            $style = $this->pdf->svgSetStyle($styles);
                ...
                switch ($nom) {
                    ...
                    case 'translate':
                        if (! isset($val[0]))
                            ...
                            $val[0] = $this->style->ConvertToMM($val[0], $this->isInDraw['w']);
                        if (! isset($val[1]))
                            $val[1] = 0.;
                        else
                            $val[1] = $this->style->ConvertToMM($val[1], $this->isInDraw['h']);
                        $actions[] = array(
                            1,
                            0,
                            0,
                    ...
                    case 'rotate':
                        ...
                        if (! isset($val[1]))
                            ...
                            $val[1] = $this->style->ConvertToMM($val[1], $this->isInDraw['w']);
                        if (! isset($val[2]))
                            $val[2] = 0.;
                        else
                            $val[2] = $this->style->ConvertToMM($val[2], $this->isInDraw['h']);
                        if ($val[1] || $val[2])
                            $actions[] = array(
                                1,
                                0,
                    ...
                    case 'matrix':
                        ...
                        if (! isset($val[4]))
                            ...
                            $val[4] = $this->style->ConvertToMM($val[4], $this->isInDraw['w']);
                        if (! isset($val[5]))
                            $val[5] = 0.;
                        else
                            $val[5] = $this->style->ConvertToMM($val[5], $this->isInDraw['h']);
                        $actions[] = $val;
                        break;
                }
            }
         ...
         *
         * @param
         *            string	titre du sommaire
         * @param
         *            int		taille en mm de la fonte du titre du sommaire
         * @param
         *            int		taille en mm de la fonte du texte du sommaire
         * @param
         *            boolean	ajouter un bookmark spécifique pour l'index, juste avant le début de celui-ci
         * @param
         *            boolean	afficher les numéros de page associés à chaque bookmark
         * @param
         *            int		si présent : page où afficher le sommaire. sinon : nouvelle page
         * @param
         *            string	nom de la fonte à utiliser
         * @return null
         */
        function createIndex($titre = 'Index', $size_title = 20, $size_bookmark = 15, $bookmark_title = true, $display_page = true, $on_page = null, $font_name = 'helvetica')
        {
            ...
            if ($old_page)
                $this->pdf->setPage($old_page);
        }

        /**
         * nouvelle page pour l'index.
         * ne pas utiliser directement. seul MyPDF doit l'utiliser !!!!
         *
         * @param
         *            int		page courante
         * @return null
         ...
         *
         * @param
         *            string code du texte
         * @return null
         */
        function textGET($key)
        {
            global $HTML2PDF_TEXTES;
            if (! isset($HTML2PDF_TEXTES[$key]))
                return '######';
         ...
         *
         * @param
         *            int		numero de l'erreur
         * @param
         *            string	nom du fichier ou c'est produit l'erreur
         * @param
         *            int		numero de la ligne ou c'est produit l'erreur
         * @param
         *            mixed	indications suplementaires sur l'erreur
         * @return string HTML eventuel associé à l'erreur
         */
        function makeError($err, $file, $line, $other = null, $html = '')
        {
            ...
            echo '<br>';
            if ($html)
                echo '<br>HTML : ...' . htmlentities($html) . '...';
            exit();
        }
    }
}
