...
class Cpdf
{
     ...
     */
    protected static $targetEncoding = 'UTF-8';
...
EOT;
    ...
    function addText($x, $y, $size, $text, $angle = 0, $wordSpaceAdjust = 0, $charSpaceAdjust = 0, $smallCaps = false)
    {
        ...
        if ($smallCaps) {
            $text = preg_replace_callback("/\p{Ll}/u", array(
                $this,
                "toUpper"
            ), $text);
        }
                    ...
                    switch ($chunkType) {
                        ...
                        case 'tRNS':
                            ...
                            if ($info['colorType'] == 3) {
                                // indexed color, rbg
                                /*
                                 * corresponding to entries in the plte chunk Alpha for palette index 0: 1 byte Alpha for palette index 1: 1 byte ...etc...
                                 */
                                ...
                                // grayscale
                                /*
                                 * corresponding to entries in the plte chunk Gray: 2 bytes, range 0 .. (2^bitdepth)-1
                                 */
                                ...
                                // truecolor
                                /*
                                 * corresponding to entries in the plte chunk Red: 2 bytes, range 0 .. (2^bitdepth)-1 Green: 2 bytes, range 0 .. (2^bitdepth)-1 Blue: 2 bytes, range 0 .. (2^bitdepth)-1
                                 */
    ...
    {
        /*
         * Todo: Pass in original filename as $imgname If already cached like image_iscached(), allow empty $img How to get w and h in this case? Then caller can check with image_iscached() whether generation of image is needed. But anyway, this function is not used!
         */
