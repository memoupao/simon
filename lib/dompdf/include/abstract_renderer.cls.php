...
abstract class Abstract_Renderer
{
    ...
    protected function _background_image($url, $x, $y, $width, $height, $style)
    {
        ...
        /*
         * //Make shorter strings with limited characters for cache associative array index - needed? //Strip common base path - server root, explicite temp, default temp; remove unwanted characters; $filedummy = strtr($filedummy,"\\:","//"); $p = strtr($_SERVER["DOCUMENT_ROOT"],"\\:","//"); $l = strlen($p); if ( substr($filedummy,0,$l) == $p) { $filedummy = substr($filedummy,$l); } else { $p = strtr(DOMPDF_TEMP_DIR,"\\:","//"); $l = strlen($p); if ( substr($filedummy,0,$l) == $p) { $filedummy = substr($filedummy,$l); } else { $p = strtr(sys_get_temp_dir(),"\\:","//"); $l = strlen($p); if ( substr($filedummy,0,$l) == $p) { $filedummy = substr($filedummy,$l); } } }
         */
    ...
    protected function _get_dash_pattern($style, $width)
    {
        ...
        switch ($style) {
            default:
            /*
             * case "solid": case "double": case "groove": case "inset": case "outset": case "ridge":
             */
            case "none":
                break;
        ...
        switch ($side) {
            ...
            case "top":
                if ($corner_style === "bevel") {
                    ...
                    $points = array(
                        $x + $left - $left_line_width,
                        $y + $top - $line_width,
                        $x + $length - $right + $right_line_width,
                        $y + $top - $line_width,
                        $x + $length - $right,
                        $y + $top,
                        $x + $left,
            ...
            case "left":
                if ($corner_style === "bevel") {
                    ...
                    $points = array(
                        $x + $left - $line_width,
                        $y + $top - $top_line_width,
                        $x + $left - $line_width,
                        $y + $length - $bottom + $bottom_line_width,
                        $x + $left,
                        $y + $length - $bottom,
                        $x + $left,
                        $y + $top
