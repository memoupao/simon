...
class Font_Metrics
{
    ...
    static function get_font($family, $subtype = "normal")
    {
        ...
        /*
         * Allow calling for various fonts in search path. Therefore not immediately return replacement on non match. Only when called with NULL try replacement. When this is also missing there is really trouble. If only the subtype fails, nevertheless return failure. Only on checking the fallback font, check various subtypes on same font.
         */
        if ($family) {
    ...
    static function get_system_fonts()
    {
        ...
        /*
         * $matches = array_intersect(array("times", "times new roman"), $keys); $names["serif"] = $names[reset($matches)]; $matches = array_intersect(array("helvetica", "arial", "verdana"), $keys); $names["sans-serif"] = $names[reset($matches)]; $matches = array_intersect(array("courier", "courier new"), $keys); $names["monospace"] = $names[reset($matches)]; $names["fixed"] = $names[reset($matches)];
         */
