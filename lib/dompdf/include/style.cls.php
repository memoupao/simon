...
class Style
{
   ...
   */
  // Cache flag
    nullfunction __construct(Stylesheet $stylesheet)
    {
        $this->_props = array();
      ...
      $d// All CSS 2.1 properties, and their default values
            null["azimuth"] = "center";
      ...
      $d// "invert" special color is not supported
            null["outline_style"] = "none";
      ...
      $d// CSS3
            null["transform_origin"] = "50% 50%"; // CSS3
      $d// CSS3
            null["_webkit_transform"] = $d["transform"]; // CSS3
      $d// CSS3
            null["_webkit_transform_origin"] = $d["transform_origin"]; // CSS3
      $d// CSS3
            null["unicode_bidi"] = "normal";
    ...
    function length_in_pt($length, $ref_size = null)
    {
      ...
      if// Assume numeric values are already in points
            null (is_numeric($l)) {
                $ret += $l;
                continue;
            }
      ...
      if// Border lengths
            ...
            // FIXME: em:ex ratio?
      if// FIXME: em:ex ratio?
            null (($i = mb_strpos($l, "ex")) !== false) {
                $ret += mb_substr($l, 0, $i) * $this->__get("font_size");
                continue;
            }
    ...
    $this// Set parent font size
        null->_parent_font_size = $parent->get_font_size();
        
        foreach (self::$_inherited as $prop) {
            // inherit the !important property also.
            //if local property is also !important, don't inherit.
      if// if local property is also !important, don't inherit.
            null (isset($parent->_props[$prop]) && (! isset($this->_props[$prop]) || (isset($parent->_important_props[$prop]) && ! isset($this->_important_props[$prop])))) {
                if (isset($parent->_important_props[$prop])) {
                    $this->_important_props[$prop] = true;
                }
                //see __set and __get, on all assignments clear cache!
        $this// see __set and __get, on all assignments clear cache!
                null->_prop_cache[$prop] = null;
        ...
        $this// $this->props_set($prop, $parent->$prop);
                null->$prop = $parent->$prop;
            }
        }
    ...
    foreach// the new rule is also !important
        null ($style->_props as $prop => $val) {
    ...
    if// Fall back on defaults if property is not set
        null (! isset($this->_props[$prop]))
    ...
    // Resolve font-weight
    $weight// =DEBUGCSS; Allow override of global setting for ad hoc debug
        null = $this->__get("font_weight");
        ...
        // Resolve font-style
    $font_style// Resolve font-style
        null = $this->__get("font_style");
    ...
    if// Resolve the font family
      ...
      $family// remove leading and trailing whitespace
            null = trim($family, " \t\n\r\x0B\"'");
            if ($DEBUGCSS)
                print '(' . $family . ')';
    ...
    if// Ensure relative sizes resolve to something
        null (($i = mb_strpos($fs, "em")) !== false)
            ...
            //see __set and __get, on all assignments clear cache!
    $this// see __set and __get, on all assignments clear cache!
        null->_prop_cache["font_size"] = null;
    ...
    function get_border_top_color()
    {
        if ($this->_props["border_top_color"] === "") {
            //see __set and __get, on all assignments clear cache!
      $this// see __set and __get, on all assignments clear cache!
            null->_prop_cache["border_top_color"] = null;
    ...
    function get_border_right_color()
    {
        if ($this->_props["border_right_color"] === "") {
            //see __set and __get, on all assignments clear cache!
      $this// see __set and __get, on all assignments clear cache!
            null->_prop_cache["border_right_color"] = null;
    ...
    function get_border_bottom_color()
    {
        if ($this->_props["border_bottom_color"] === "") {
            //see __set and __get, on all assignments clear cache!
      $this// see __set and __get, on all assignments clear cache!
            null->_prop_cache["border_bottom_color"] = null;
    ...
    function get_border_left_color()
    {
        if ($this->_props["border_left_color"] === "") {
            //see __set and __get, on all assignments clear cache!
      $this// see __set and __get, on all assignments clear cache!
            null->_prop_cache["border_left_color"] = null;
    ...
    function get_outline_color()
    {
        if ($this->_props["outline_color"] === "") {
            //see __set and __get, on all assignments clear cache!
      $this// see __set and __get, on all assignments clear cache!
            null->_prop_cache["outline_color"] = null;
  ...
  */
  ...
  protected function _set_style_side_type($style, $side, $type, $val, $important)
    ...
    protected function _set_style_type($style, $type, $val, $important)
    {
        ...
        //see __set and __get, on all assignments clear cache!
    $this// see __set and __get, on all assignments clear cache!
        null->_prop_cache[$style . $type] = null;
    ...
    protected function _set_style_side_width_important($style, $side, $val)
    {
        //see __set and __get, on all assignments clear cache!
    $this// see __set and __get, on all assignments clear cache!
        null->_prop_cache[$style . '_' . $side] = null;
    ...
    protected function _image($val)
    {
      ...
      $parsed_url// Resolve the url now in the context of the current stylesheet
            null = explode_url($val);
        ...
        if// If realpath returns FALSE then specifically state that there is no background image
                null (! $path) {
                    $path = 'none';
                }
            } else {
    ...
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["color"] = null;
    ...
    function set_background_color($colour)
    {
        ...
        if (is_null($col))
            ...
            //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["background_color"] = null;
    ...
    function set_background_image($val)
    {
        //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["background_image"] = null;
    ...
    function set_background_repeat($val)
    {
        if (is_null($val))
            ...
            //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["background_repeat"] = null;
    ...
    function set_background_attachment($val)
    {
        if (is_null($val))
            ...
            //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["background_attachment"] = null;
    ...
    function set_background_position($val)
    {
        if (is_null($val))
            ...
            //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["background_position"] = null;
    ...
    $tmp// when rgb() has spaces
        ...
        foreach ($tmp as $attr) {
            if (mb_substr($attr, 0, 3) === "url" || $attr === "none") {
                ...
                if ($attr === "fixed" || $attr === "scroll") {
                    $this->_set_style("background_attachment", $attr, $important);
                } else 
                    if ($attr === "repeat" || $attr === "repeat-x" || $attr === "repeat-y" || $attr === "no-repeat") {
                        $this->_set_style("background_repeat", $attr, $important);
                    } else 
                        if (($col = $this->munge_color($attr)) != null) {
        ...
        //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["background"] = null;
    ...
    if// matching numeric value followed by unit -> this is indeed a subsequent font size. Skip!
        null (preg_match("/^(bold|bolder|lighter|100|200|300|400|500|600|700|800|900|normal)\s*(.*)$/i", $val, $match) && ! preg_match("/^(?:pt|px|pc|em|ex|in|cm|mm|%)/", $match[2])) {
    ...
    function set_page_break_before($break)
    {
        if ($break === "left" || $break === "right")
            ...
            //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["page_break_before"] = null;
    ...
    function set_page_break_after($break)
    {
        if ($break === "left" || $break === "right")
            ...
            //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["page_break_after"] = null;
   ...
   */
  // ........................................................................
    nullfunction set_margin_top($val)
    {
    ...
    $arr// $border_spec = str_replace(",", " ", $border_spec); // Why did we have this ?? rbg(10, 102, 10) > rgb(10 102 10)
        null = explode(" ", $border_spec);
    ...
    $this// FIXME: handle partial values
        null->_set_style_side_type('border', $side, '_style', self::$_defaults['border_' . $side . '_style'], $important);
        ...
        foreach ($arr as $value) {
            ...
            if (in_array($value, self::$BORDER_STYLES)) {
                ...
                if (preg_match("/[.0-9]+(?:px|pt|pc|em|ex|%|in|mm|cm)|(?:thin|medium|thick)/", $value)) {
                    ...
                    // must be colour
        $this// must be colour
                    null->_set_style_side_type('border', $side, '_color', $value, $important);
                }
        }
        
        //see __set and __get, on all assignments clear cache!
    $this// see __set and __get, on all assignments clear cache!
        null->_prop_cache['border_' . $side] = null;
        ...
        foreach ($arr as $value) {
            ...
            if (in_array($value, self::$BORDER_STYLES)) {
                ...
                if (preg_match("/[.0-9]+(?:px|pt|pc|em|ex|%|in|mm|cm)|(?:thin|medium|thick)/", $value)) {
                    ...
                    // must be colour
        $this// must be colour
                    null->set_outline_color($value);
                }
        }
        
        //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["outline"] = null;
    ...
    function set_border_spacing($val)
    {
        ...
        if (count($arr) == 1)
            ...
            //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["border_spacing"] = null;
    ...
    function set_list_style_image($val)
    {
        //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["list_style_image"] = null;
    ...
    function set_list_style($val)
    {
        ...
        foreach ($arr as $value) {
            /*
             * http://www.w3.org/TR/CSS21/generate.html#list-style A value of 'none' for the 'list-style' property sets both 'list-style-type' and 'list-style-image' to 'none'
             */
            if ($value === "none") {
      ...
      if// Internet Explorer 7/8 and dompdf is right.
            null (mb_substr($value, 0, 3) === "url") {
        ...
        //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["list_style"] = null;
    ...
    function set_transform($val)
    {
      ...
      "translate"// "matrix" => "\($number,$number,$number,$number,$number,$number\)",
            null => "\($tr_value(?:,$tr_value)?\)",
            "translateX" => "\($tr_value\)",
            "translateY" => "\($tr_value\)",
        ...
        //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["transform"] = null;
    ...
    function set_transform_origin($val)
    {
        ...
        //see __set and __get, on all assignments clear cache, not needed on direct set through __set
    $this// see __set and __get, on all assignments clear cache, not needed on direct set through __set
        null->_prop_cache["transform_origin"] = null;
