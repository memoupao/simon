...
' . $content . '
        ...
        {
        	// -------------------------------------------------->
        	// DA 2.0 [09-01-2014 12:30]
        	// Solucion temporal al llamar a ip publica desde LAN local
        	// aunque esto deberia de funcionar con el /etc/hosts o iptables        	     
        	if (strstr($src,'sgp/img/FE.jpg') ) {
        		$src = realpath(dirname(__FILE__).'/../../img/FE.jpg');        		
        	}
        	// --------------------------------------------------<
                ...
                for ($y = 0; $y < count($corr); $y ++) {
                    if (isset($corr[$y][$x]) && is_array($corr[$y][$x]) && $corr[$y][$x][2] > 1) {
                        ...
                        for ($i = 0; $i < $corr[$y][$x][2]; $i ++)
                            // -------------------------------------------------->
                            // DA 2.0 [24-11-2013 15:47]
                            // Correccion cuando no existe el indice
                            if(isset($sw[$x + $i])) {
                                $s += $sw[$x + $i];
                            }
                            // --------------------------------------------------<
