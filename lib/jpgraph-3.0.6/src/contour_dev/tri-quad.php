...
class SingleTestTriangle
{
    ...
    {
        $this->labels[$this->GetNextHigherContourIdx($v1)][] = array(
            $x1,
            $y1,
            $v1,
            $angle
    ...
    function annotate($x1, $y1, $x2, $y2, $x1p, $y1p, $v1, $v2, $v1p)
    {
        if (! $this->showannotation)
            return;
            /*
         * $this->g->img->SetColor('green'); $this->g->img->FilledCircle($this->scale->TranslateX($x1),$this->scale->TranslateY($y1), 4); $this->g->img->FilledCircle($this->scale->TranslateX($x2),$this->scale->TranslateY($y2), 4); $this->g->img->SetColor('red'); $this->g->img->FilledCircle($this->scale->TranslateX($x1p),$this->scale->TranslateY($y1p), 4);
         */
        ...
        if ($depth >= self::$maxdepth) {
            ...
            if ($vv1 == $vv2 && $vv2 == $vv3 && $vv3 == $vv4) {
                ...
                if (($fcnt == 2 && $totdv == 2) || ($fcnt == 4 && $totdv == 4)) {
                    ...
                    // Top left
                    $this->RectFill(($v1 + $v2) / 2, $v2, ($v2 + $v3) / 2, $vc, $x1, $yc, $x2, $y2, $xc, $y2, $xc, $yc, $depth + 1);
                    // Top right
                    $this->RectFill($vc, ($v2 + $v3) / 2, $v3, ($v3 + $v4) / 2, $xc, $yc, $xc, $y2, $x3, $y3, $x3, $yc, $depth + 1);
                    
                    // Bottom left
                    $this->RectFill($v1, ($v1 + $v2) / 2, $vc, ($v1 + $v4) / 2, $x1, $y1, $x1, $yc, $xc, $yc, $xc, $y4, $depth + 1);
                    
                    // Bottom right
                    $this->RectFill(($v1 + $v4) / 2, $vc, ($v3 + $v4) / 2, $v4, $xc, $y1, $xc, $yc, $x3, $yc, $x4, $y4, $depth + 1);
                }
            }
        }
    }
    ...
    function TriFill($v1, $v2, $v3, $x1, $y1, $x2, $y2, $x3, $y3, $depth)
    {
        if ($depth >= self::$maxdepth) {
            ...
            if ($vv1 == $vv2 && $vv2 == $vv3) {
                ...
                if ($fcnt == 2 && ((abs($v1p - $v2p) < $eps && $dv1 == 1 && $dv2 == 1) || (abs($v1p - $v3p) < $eps && $dv1 == 1 && $dv3 == 1) || (abs($v2p - $v3p) < $eps && $dv2 == 1 && $dv3 == 1))) {
