...
class Findpolygon
{
    ...
    function SetupTestData()
    {
        ...
        for ($c = 0; $c < count($p); $c ++) {
            ...
            $k = 1;
            for ($i = 0; $i < ($n - 4) / 2; $i ++, $k ++) {
                $this->contourCoord[$c][$k] = array(
                    $this->contourCoord[$c][$k - 1][1],
                    array(
                        $p[$c][2 * $k + 2],
                        $p[$c][2 * $k + 1 + 2]
                    )
                        ...
                        do {
                            ...
                            for ($i = 0; $i < $nn && ! $found; ++ $i) {
                                ...
                                if ($found = ($x1 == $edge[0][0] && $y1 == $edge[0][1])) {
                                    array_unshift($polygons[$pol], array(
                                        $edge[1][0],
                                        $edge[1][1]
                                    ));
                                    $x2 = $x1;
            ...
            for ($row = 0; $row < $ny_vertices - 1; ++ $row) {
                ...
                if ($n == 0) {
                    ...
                    if ($this->edges[HORIZ_EDGE][$row][$col]) {
                        if ($this->edges[HORIZ_EDGE][$row + 1][$col])
                            $type = 0; // North-South
                        elseif ($this->edges[VERT_EDGE][$row][$col])
                            $type = 2;
                        elseif ($this->edges[VERT_EDGE][$row][$col + 1])
                            $type = 3;
