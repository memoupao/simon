...
function ListaNoObjecionCompras()
{
    $objProy = new BLProyecto();
    $idProy = $_GET['idProy'];
    
    $rs = $objProy->NoObjecionCompra_Listado($idProy);
    $fields = $objProy->iGetArrayFields($rs);
    $objFun = new Functions();
    $xmlResult = $objFun->iGenerateXML($rs, "noobjecioncompra", $fields);
    return $xmlResult;
}
