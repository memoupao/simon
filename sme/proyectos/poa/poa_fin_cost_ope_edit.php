 ...
 <?php } ?>
  <div style="width: 99%; border: solid 1px #D3D3D3;">
						...
						<table width="516" align="center" class="TableEditReg">
							...
							<tr valign="baseline">
								<td width="91" height="24" align="left" valign="middle"
								...
								<td colspan="5" align="left" valign="middle" nowrap="nowrap"><input
									...
									size="2" maxlength="5" /> <input name="t09_sub" type="text"
									class="costosoperativos" id="t09_sub"
									value="<?php echo($row["t09_sub"]);?>" size="75"
									maxlength="100" /></td>
							</tr>
							<tr valign="baseline">
								<td height="22" align="left" valign="middle" nowrap="nowrap"><strong>Categoria
										Gasto</strong></td>
        ...
        ?>
							...
							<tr valign="baseline">
								<td height="27" align="left" valign="middle" nowrap="nowrap"><strong>Gasto</strong></td>
							...
							<tr>
								...
								<td width="79" align="left" valign="middle"><input name="t10_cu"
									...
									value="<?php echo($rowCosteo["t10_cu"]);?>"
									size="12" /></td>
								...
								<td valign="middle"><input name="t10_cant" type="text"
									...
									style="text-align: center;" onkeyup="TotalizarPresup();"
									value="<?php echo($rowCosteo["t10_cant"]);?>"
									size="10" /></td>
							...
							<tr>
								<td colspan="6" align="center" class="TextDescripcion">
      <?php
    $ObservaMF = $rowCosteo['t10_obs_mf'];
    ?>
      <?php if($objHC->MF==$ObjSession->PerfilID || $objHC->CMF==$ObjSession->PerfilID || $objHC->Admin==$ObjSession->PerfilID) { ?>
          <fieldset>
										<legend>Observaciones de Monitoreo Financiero</legend>
										<textarea name="txtobs_mf" cols="100" rows="2" id="txtobs_mf"
											class="costosoperativos"><?php echo($ObservaMF);?></textarea>
									</fieldset>
          <?php } ?>
          <?php if($objHC->Ejec==$ObjSession->PerfilID) { ?>
          <input type="hidden" name="txtobs_mf" id="txtobs_mf"
									class="costosoperativos" value="<?php echo($ObservaMF);?>" />
									<fieldset>
										<legend>Observaciones de Monitoreo Financiero</legend>
										<span style="text-align: justify; color: #F00;">
          <?php echo(nl2br($ObservaMF));?>
          </span>
									</fieldset>
          <?php } ?>
      </td>
  ...
  function Guardar_CostoOperativo()
	...
	<?php $ObjSession->AuthorizedPage(); ?>
	 ...
	 if( $('#cbocatgasto').val()=="" ) {alert(getAlertText("Seleccione Categoría de Gasto")); $('#cbocatgasto').focus(); return false;}
	 if( $('#t10_descrip').val()=="" ) {alert(getAlertText("Ingrese Descripción del Gasto")); $('#t10_descrip').focus(); return false;}
	 if( $('#t10_um').val()=="" ) {alert(getAlertText("Ingrese Unidad de Medida")); $('#t10_um').focus(); return false;}
 	 if( $('#t10_cu').val()=="" ) {alert(getAlertText("Ingrese Costo Unitario")); $('#t10_cu').focus(); return false;}
 	 if( isNaN(parseFloat($('#t10_cu').val().trim())) ) {alert(getAlertText("Costo Unitario debe ser numérico")); $('#t10_cu').focus(); return false;}
	 if( $('#t10_cant').val()=="" ) {alert(getAlertText("Ingrese Cantidad")); $('#t10_cant').focus(); return false;}
 	 if( isNaN(parseFloat($('#t10_cant').val().trim())) ) {alert(getAlertText("Cantidad debe ser numérico")); $('#t10_cant').focus(); return false;}
	 if( $('#t09_meta').val()=="" ) {alert(getAlertText("Ingrese Meta Total")); $('#t09_meta').focus(); return false;}
 	 if( isNaN(parseFloat($('#t09_meta').val().trim())) ) {alert(getAlertText("Meta Total debe ser numérico")); $('#t09_meta').focus(); return false;}
	...
	function CNumber(str)
	  {
		 ...
		 return numero;
					...
					<script>
  $("#t10_cant").numeric().pasteNumeric();
  $("#t10_cu").numeric().pasteNumeric();
  $('#t10_cost_parc').numeric().pasteNumeric();
  $('#t09_meta').numeric().pasteNumeric();
  $('#t10_cost_tot').numeric().pasteNumeric();
  TotalizarPresup();
  </script>
