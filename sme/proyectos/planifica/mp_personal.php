			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td width="25%"><button class="Button"
							onclick="LoadPersonal(); return false;" value="Recargar Listado">
      ...
      <tr class="RowData" style="background-color: #FFF;">
		...
		<?php if(!$modificar){ ?>
			<a href="#"><img src="../../../img/financ.gif" alt="" width="16"
									...
									onclick="LoadPersonalFTE('<?php echo($row["codigo"]);?>'); return false;" /></a>	
		<?php } ?>
		</span></td>
...
{
	var url = "mp_personal_edit.php?mode=<?php echo(md5("ajax_new"));?>&idProy=<?php echo($idProy);?>&idVersion=<?php echo($idVersion);?>";
	loadPopup("Retribuciones del Personal", url);
}
function EditarPersonal(idPer)
{
	var url = "mp_personal_edit.php?mode=<?php echo(md5("ajax_edit"));?>&idProy=<?php echo($idProy);?>&idVersion=<?php echo($idVersion);?>&idPer="+idPer;
	loadPopup("Retribuciones del Personal", url);
}
function LoadPersonalFTE(idPer)
{
	var url = "mp_personal_fte.php?mode=<?php echo(md5("ajax_fte"));?>&idProy=<?php echo($idProy);?>&idVersion=<?php echo($idVersion);?>&idPer="+idPer;
...
function EliminarPersonal(idPer, Cargo)
{
	...
	{
		var BodyForm = "t02_cod_proy="+$('#txtCodProy').val()+"&t20_version="+$('#cboversion').val()+"&t03_id_per="+idPer;
		var sURL = "mp_personal_process.php?action=<?php echo(md5("ajax_del"))?>";
...
function PersonalCall(req)
  {
	...
	if(ret=="Exito")
	{ 	alert(respuesta.replace(ret,"")); LoadPersonal(); }
	else
	{alert(respuesta);}  
  }
