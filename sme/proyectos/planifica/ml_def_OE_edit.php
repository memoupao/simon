...
$urlListado = "ml_def_OE.php?idProy=$idProy&idVersion=$idVersion";
...
if ($proc == "save") {
    ...
    if ($view == "edit") {
        ...
        $ReturnPage = $objML->ActualizarDefinicionOE();
        
        $objML = NULL;
    }
    if ($view == "new") {
        $objML = new BLMarcoLogico();
        $ReturnPage = $objML->NuevoDefinicionOE();
        $objML = NULL;
    }
...
if ($proc == md5("del")) {
    ...
    $ReturnPage = $objML->EliminarDefinicionOE($idProy, $idVersion, $id);
    $objML = NULL;
    $Function->Redirect($urlListado . "&error=" . $objML->Error);
}

$urlSave = "ml_def_OE_edit.php?idProy=$idProy&idVersion=$idVersion";
...
$row = 0;
if ($view == "edit") {
    $idDefinicion = $Function->__GET('id');
    $objML = new BLMarcoLogico();
    $row = $objML->GetDefinicionOE($idProy, $idVersion, $idDefinicion);
    $objML = NULL;
}
?>
...
if ($view == "new") {
    $objFunc->SetSubTitle("Componentes  -  Nuevo Registro");
} else {
    $objFunc->SetSubTitle("Componentes -  Editar Registro");
}

?>
<title>Definicion Objetivo Especifico</title>
<style type="text/css">
<!--
#Layer1 {
	position: absolute;
...
<form id="frmMain" name="frmMain" method="post" action="#">
		...
		<table align="center" class="TableEditReg">
			<tr valign="baseline">
				...
				<td width="598" colspan="5" align="left"><input name="t08_cod_comp"
					type="text" id="t08_cod_comp"
					value="<?php echo($row["t08_cod_comp"]);?>" size="4" maxlength="5"
					readonly></td>
			</tr>


			<tr valign="baseline">
				<td nowrap align="left" valign="top">Definicion</td>
				<td colspan="5" align="left"><textarea name="t08_comp_desc"
						cols="85" rows="9" id="t08_comp_desc" <?php if (($view == "edit") && !empty($lsDisabled)) echo 'readonly';?>><?php echo($row["t08_comp_desc"]);?></textarea>
				</td>
			</tr>
	...
	<script language="javascript" type="text/javascript">
	  ...
	  function Guardar()
	  {
	  	 ...
	  	 var formulario = document.getElementById("frmMain") ;
	  	 <?php if ($view != "edit") { ?>
 		 if(formulario.t08_comp_desc.value=="")
		 {
		 	alert("Ingrese Definicion"); 
			formulario.t08_comp_desc.focus();
		 	return false ;
		 }
		 <?php } ?>
...
<?php
mysql_free_result($rsDefinicion);
?>
