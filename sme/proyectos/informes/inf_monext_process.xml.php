...
if (md5("lista_inf_se") == $Accion) {
    echo (listarInfSE());
    exit();
}

if (md5("lista_inf_se_vb") == $Accion) {
    echo (listarInformesSupSinVB());
    exit();
}

/* Definición de Funciones que escriben el archivo XML */
function listarInfSE()
{
    $objInf = new BLInformes();

    $rs = $objInf->listarInfSE($_GET['idProy'], $_GET['idVersion']);
...
function listarInformesSupSinVB()
{
    ...
    $rs = $objInf->listarInformesSupSinVB();
    $fields = $objInf->iGetArrayFields($rs);
