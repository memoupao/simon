		...
		<div id="toolbar" style="height: 4px;" class="BackColor">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td width="10%"><button id="btn_Guardar" class="Button btn_guardar" onclick="Guardar_InformeEntregableCab(); return false;">Guardar</button></td>
					<td width="12%"><button class="Button" onclick="btnCancelar_Clic(); return false;">Cerrar y Volver</button></td>
					<?php if(!$isSE || $row['vb_se']!=1){?>
					<td width="15%"><button class="Button btn_guardar" onclick="Enviar_Correcion(); return false;" id="btn_ECorrecion">Enviar a Corrección</button></td>
					<?php }?>
					<?php if($IsMF){?>
					<td width="15%"><button class="Button btn_guardar" onclick="Enviar_Aprobacion(); return false;" id="btn_AprobarInf">Dar V°B°</button></td>
					<?php }?>
					<td>
					<?php if($isSE || $IsMF){?>
        ...
        <div>
			<div id="divCabeceraInforme">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="TableEditReg">
                    ...
                    <tr>
						<td colspan="2">
							<font style="font-weight: bold; color: #48628A;">Introducción </font> <br /> 
							<textarea name="txtIntro" cols="2500" rows="2" id="txtIntro" style="padding: 0px; width: 100%;"><?php echo($row['intro_gp']);?></textarea>
						</td>
						<td colspan="2">
							<font style="font-weight: bold; color: #48628A;">Observaciones del Gestor de Proyectos </font> <br /> 
							...
							<textarea name="txtObs" cols="2500" rows="2" id="txtObs" style="padding: 0px; width: 100%;"><?php echo($row['obs_gp']);?></textarea>
						</td>
...
if ($mostrar) {
    ...
    var estado = $("#t25_estado").val();
	var Elaboracion= "<?php echo($HC->EstInf_Ela);?>";
	var AprobadoMT = "<?php echo($HC->EstInf_Aprob);?>";
	var Correccion = "<?php echo($HC->EstInf_Corr);?>";
	var Revision   = "<?php echo($HC->EstInf_Rev);?>";
	...
	function Enviar_CorrecionCallback(req)
	{
	  ...
	  {
		    dsLista.loadData();
            btnEditar_Clic('<?php echo($idProy);?>', '<?php echo($idVersion);?>', $('#anio').val(), $('#idEntregable').val());
		    alert(respuesta.replace(ret,""));
		    $('#btn_ECorrecion, #btn_AprobarInf, #btn_Guardar').hide();
	  }
	  else
	  {alert(respuesta);}
	}
    ...
    function Guardar_InformeEntregableCab()
	{
    	<?php $ObjSession->AuthorizedPage('EDITAR'); ?>
    	...
    	var BodyForm="cboanio="+anio+"&anio="+$("#anio").val()+"&cboentregable="+entregable+"&idEntregable="+$("#idEntregable").val()+"&periodo="+per+"&fchPresentacion="+$("#fchPresentacion").val()+"&estadoInf="+est+"&idProy=<?php echo($idProy);?>&obs_gp="+$('#txtObs').val()+"&idVersion="+$("#idVersion").val()+"&vb_se="+checkbox+"&intro_gp="+$('#txtIntro').val();

    	var sURL = "inf_entregable_process.php?action=<?php echo(md5("ajax_edit_vb"));?>";
	...
	function informeSuccessCallback(req)
	{
        ...
        {
            dsLista.loadData();
    		var vs = respuesta.substring(0,5);
