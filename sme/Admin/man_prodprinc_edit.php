...
$idTabla = $objFunc->__GET('idtabla');
$idTablaAux = $objFunc->__GET('idtablaaux');

$row = 0;

if ($view == md5("ajax_edit")) {
    $objFunc->SetSubTitle("Editando Producto principal");
    $id = $objFunc->__GET('id');
    $row = $objTablas->SubTipoSeleccionar($id);
} else {
    $row = 0;
    $objFunc->SetSubTitle("Nuevo Producto principal ");
}
?>
...
<body class="oneColElsCtrHdr">
 ...
 <?php } ?>
								...
								<td width="542">
									<fieldset>
										...
										<table width="100%" border="0" cellspacing="1" cellpadding="0">
											...
											<tr>
												<td height="28">Tipo</td>
												<td><select name="cbotablaaux" id="cbotablaaux"
													style="width: 280px">
														<option value=""></option>
            <?php
            $idAux = $row['idTablaAux'];
            if ($idAux == "") {
                $idAux = $idTablaAux;
            }
            $rs = $objTablas->ListadoTipos($idTabla);
            $objFunc->llenarComboI($rs, 'codigo', 'descripcion', $idAux);
            ?>
            </select></td>
												<td>&nbsp;</td>
											</tr>
  ...
  function btnGuardar_Clic()
	{
	 if( $('#txtnombre').val()=="" ) {alert("Ingrese Descripcion del Tipo"); return false;}
	...
	// --------------------------------------------------<
	 ...
	 var sURL = "man_subtipos_process.php?action=<?php echo($view);?>"
