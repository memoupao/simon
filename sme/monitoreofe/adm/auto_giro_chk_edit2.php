...
if ($view == md5("ajax_edit")) {
    $row = $objFE->DesembolsoME_Seleccionar($idProy, $idVisita, $idAprob, $idDesemb);
 ...
 <?php } ?>
							...
							<table border="0" cellpadding="0" cellspacing="0">
								<tr>
									<td width="9%"><button class="Button"
											...
											<?php if($ObjSession->PerfilID!=$HC->FE) {echo('disabled="disabled"');} ?>>Guardar
										</button></td>
									<td width="9%"><button class="Button"
											onclick=" viewDesembolso('<?php echo($idProy);?>', '<?php echo($idVisita);?>', '<?php echo($idAprob);?>'); return false;"
  ...
  function GuardarDesembolso()
	...
	<?php $ObjSession->AuthorizedPage(); ?>	
	 ...
	 var monto_aprobado  = Math.round(CNumber($('#txtmontoaprobado').val()),2);
	 var monto_desembolsado_fecha  = Math.round(CNumber($('#txtmontodesembolsado').val()),2);
	 var monto_desembolsar  = Math.round(CNumber($('#txtmontodesemb').val()),2);
	 
	 if(monto_desembolsar <=0 ) {alert("Ingrese un Monto mayor a 0"); return; }
	 
	 if( (monto_desembolsar + monto_desembolsado_fecha) > monto_aprobado)
	 {
	 ...
	 var sURL = "auto_giro_chk_process.php?action=<?php echo(md5("guardar"));?>&mode=<?php echo($view);?>" ;
