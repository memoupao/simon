...
if ($idInforme == "") {
    $objFunc->MensajeError("No se ha Establecido el Periodo de Referencia para el Informe");
    $idComp = - 1;
}
?> 
<div id="divTableLista">
    ...
    $iRsAct = $objInf->Inf_UA_ListadoActividades2($idProy, $idComp, $idInforme);
    ...
    <?php
        $iRs = $objInf->Inf_UA_ListadoSubActividades2($idProy, $idComp, $idAct, $idInforme);
    ...
    <tr>
          ...
          <input name="txt_cod_sub[]" id="txt_cod_sub[]" type="hidden"
							value="<?php echo($rowsub['codigo']); ?>" class="fisico" /></td>
						...
						<td align="center"><?php echo($porcEjecucion);?>%</td>
						<td align="center"><textarea name="txtcoment_fisico[]" rows="2"
								id="txtcoment_fisico[]" style="width: 100%; height: 100%"
								class="fisico"><?php echo($rowsub['observaciones']);?></textarea></td>
					</tr>
    ...
    <tfoot>
			...
			<input type="hidden" name="t02_cod_proy"
				value="<?php echo($idProy);?>" class='fisico' /> <input
				type="hidden" name="t08_cod_comp" value="<?php echo($idComp);?>"
				class='fisico' /> <input type="hidden" name="t55_num" id="t55_num"
				value="<?php echo($idInforme);?>" class='fisico' /> <input
				type="hidden" name="idFuente" id="idFuente"
				value="<?php echo($idFuente);?>" class='presup' />
			<script language="JavaScript" type="text/javascript">
	function ShowActividadFis(idAct)
	 {
		 var id = "#" + idAct ;
		 $(id).toggle();
   		 return false;
