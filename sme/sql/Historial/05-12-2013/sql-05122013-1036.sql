/*
// -------------------------------------------------->
// AQ 2.0 [05-12-2013 11:52]
// Creación de tabla para los Gastos de Supervisión
// de Proyectos
// --------------------------------------------------<
*/
CREATE TABLE IF NOT EXISTS `t03_mp_gas_supervision` (
...
ALTER TABLE `t03_mp_gas_supervision`
  ADD CONSTRAINT `t03_mp_gas_supervision_ibfk_1` FOREIGN KEY (`t02_cod_proy`, `t02_version`) REFERENCES `t02_dg_proy` (`t02_cod_proy`, `t02_version`) ON DELETE CASCADE ON UPDATE CASCADE;

/*
// -------------------------------------------------->
// AQ 2.0 [05-12-2013 11:02]
...
trans1 : BEGIN
    ...
    /* Control de las Características de los Indicadores de Producto */
    DELETE FROM t09_act_ind_car_ctrl WHERE t02_cod_proy=_proy AND t02_version=_ver AND t09_car_anio=_anio;
    ...
    /* Entregables */
    DELETE FROM t02_entregable WHERE t02_cod_proy=_proy AND t02_version=_ver AND t02_anio=_anio;
...
/*
// -------------------------------------------------->
// AQ 2.0 [05-12-2013 12:03]
// Datos resumidos de Manejo de Proyecto por Fuente
// --------------------------------------------------<
*/
...
SELECT 
    ...
    `fte`.`t03_id_per` AS `rubro`,_utf8'Personal del Proyecto' AS `nomactividad`,`fte`.`t03_id_inst` AS `idinst`,
...
union all SELECT 
    ...
    `fte`.`t03_id_equi` AS `rubro`,_utf8'Equipamiento del Proyecto' AS `nomactividad`,`fte`.`t03_id_inst` AS `idinst`,
    `i`.`t01_sig_inst` AS `siglas`,`fte`.`t03_monto` AS `aporte_fuente` 
...
union all select 
    ...
    `p`.`cod_ext` AS `codsub`,`c`.`cod_ext` AS `rubro`,_utf8'Gastos de Funcionamiento' AS `nomactividad`,
...
union all select 
    ...
    NULL AS `codsub`,NULL AS `rubro`,_utf8'Gastos Administrativos' AS `nomactividad`,`fte`.`t03_id_inst` AS `idinst`,
    `i`.`t01_sig_inst` AS `siglas`,`fte`.`t03_monto` AS `aporte_fuente` 
...
union all select 
        ...
        NULL AS `codsub`,NULL AS `rubro`,_utf8'Linea de Base' AS `nomactividad`,`fte`.`t03_id_inst` AS `idinst`,
        `i`.`t01_sig_inst` AS `siglas`,`fte`.`t03_monto` AS `aporte_fuente` 
...
union all select 
        ...
        NULL AS `codsub`,NULL AS `rubro`,_utf8'Imprevistos' AS `nomactividad`,`fte`.`t03_id_inst` AS `idinst`,
        `i`.`t01_sig_inst` AS `siglas`,`fte`.`t03_monto` AS `aporte_fuente` 
...
union all select 
        ...
        NULL AS `codsub`,NULL AS `rubro`,_utf8'Gastos de Supervision' AS `nomactividad`,`fte`.`t03_id_inst` AS `idinst`,
        `i`.`t01_sig_inst` AS `siglas`,`fte`.`t03_monto` AS `aporte_fuente` 
...
// --------------------------------------------------<
*/
DROP FUNCTION IF EXISTS `fn_costos_indirectos`;

DELIMITER $$

CREATE FUNCTION `fn_costos_indirectos`(_proy varchar(10), _ver int) RETURNS double
    DETERMINISTIC
BEGIN
    ...
    /* Linea de Base */
    -- SELECT fn_mp_linea_base( _proy, _ver) INTO _linea_base;
     SELECT SUM(IFNULL(t03_monto,0)) 
      INTO _linea_base
    ...
    /* Imprevistos */
    -- SELECT fn_mp_imprevistos( _proy, _ver) INTO _imprevistos;    
     SELECT SUM(IFNULL(t03_monto,0)) 
      INTO _imprevistos
...
// AQ 2.0 [05-12-2013 11:25]
// Inclusión de Gastos de Supervisión de Proyectos
// Obs: Está asociado a tirmiestres.
// --------------------------------------------------<
*/
DROP FUNCTION IF EXISTS `fn_costos_indirectos_pre`;
...
/*
// -------------------------------------------------->
// AQ 2.0 [05-12-2013 12:12]
// Inclusión de Gastos de Supervisión de Proyectos
// --------------------------------------------------<
*/
DROP PROCEDURE IF EXISTS `sp_sel_mp_linea_base_imprevistos`;

DELIMITER $$

CREATE PROCEDURE `sp_sel_mp_linea_base_imprevistos`(_proy VARCHAR(10), _ver INT)
BEGIN
DECLARE _linea_base double;
DECLARE _imprevistos double;
DECLARE _supervision double;
DECLARE _codfte   INT ;
select 10 into _codfte  ; -- > Codigo de Fondoempleo
