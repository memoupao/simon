...
$rowInf = $objInf->InformeFinancieroListado($idProy);
...
a {
			...
			<table width="99%" align="center" cellpadding="0" cellspacing="0">
				<thead>
					<tr>
						...
						<td rowspan="2" align="center" valign="middle">PERIODO REFERENCIA</td>
						<td rowspan="2" align="center" valign="middle">FECHA DE
							PRESENTACIóN</td>
						<td rowspan="2" align="center" valign="middle">ESTADO</td>
						<td rowspan="2" align="center" valign="middle">MONTO INFORMADO</td>
						<td colspan="2" align="center" valign="middle">% CUMPLIMIENTO</td>
						<td rowspan="2" align="center" valign="middle">OBSERVACIONES</td>
					</tr>
					<tr>
    ...
    while ($rInforme = mysqli_fetch_assoc($rowInf)) {
        ...
        $anio = split(" ", $rInforme['anio']);
        $mes = split(" ", $rInforme['mes']);
        
        $cumpli = 0;
        $acum = 0;
        $contador = 0;
        $rowCumpli = $objInf->RptCumplimientoFinanc_mes($idProy, $anio[1], $mes[1]);
        $rowCumpliAcum = $objInf->RptCumplimientoFinanc_mesAcumulado($idProy, $anio[1], $mes[1]);
        
        while ($rowCum = mysqli_fetch_assoc($rowCumpli)) {
            if ($rowCum['meta'] != 0) {
                $contador ++;
                $cumpli += $rowCum['cumplimiento'];
            }
        }
        $rowCumpli->free();
        
        if ($contador != 0) {
            $cumpli = $cumpli / $contador;
        }
        
        $contador = 0;
        
        while ($rowCum = mysqli_fetch_assoc($rowCumpliAcum)) {
            if ($rowCum['meta'] != 0) {
                $contador ++;
                $acum += $rowCum['cumplimiento'];
            }
        }
        $rowCumpliAcum->free();
        if ($contador != 0) {
    ...
    <tr style="min-height: 35px; height: 35px;">
						...
						<td width="10%" align="center" valign="middle"><a
							...
							href"#" onclick="RepInfo('<?php echo($idProy); ?>', '<?php echo($rInforme['t40_anio']); ?>', '<?php echo($rInforme['t40_mes']); ?>')"
							title="Ver el Informe Financiero Mensual">
	  <?php echo($rInforme['fec_pre']); ?>
      </a></td>
						<td width="8%" align="center" valign="middle"><?php echo($rInforme['est_eje']); ?></td>
						<td width="8%" align="right" valign="middle"><?php echo($rInforme['monto']); ?></td>
						<td width="8%" align="right" valign="middle"
							<?php echo($semaforo_mes)?>><a
							style="color: #336; text-decoration: underline;"
							href"#" onclick="RepAvanceMes('<?php echo($idProy); ?>', '<?php echo($rInforme['t40_anio']); ?>', '<?php echo($rInforme['t40_mes']); ?>', '<?php echo($rInforme['vsinf']); ?>')"
							title="Ver Detalle del Cumplimiento"> <!--?php echo($rInforme['cump_mes']); ?-->
       <?php echo(number_format($cumpli,2)); ?>
       </a> &nbsp;&nbsp;</td>
						<td width="8%" align="right" valign="middle"
							<?php echo($semaforo_acum)?>><a
							style="color: #336; text-decoration: underline;"
							href"#" onclick="RepAvanceAcum('<?php echo($idProy); ?>', '<?php echo($rInforme['t40_anio']); ?>', '<?php echo($rInforme['t40_mes']); ?>', '<?php echo($rInforme['vsinf']); ?>')"
							title="Ver Detalle del Cumplimiento"> <!--?php echo($rInforme['cump_acum_mes']); ?-->
	  <?php echo(number_format($acum,2)); ?>
      </a> &nbsp;&nbsp;</td>
						<td width="37%" align="left" valign="middle"><?php echo(nl2br( substr($rInforme['observaciones'],0,90 ))); ?></td>
					</tr>
	<?php
    } // End While
    $rowInf->free();
	...
	{
		var params = "idProy="+idproy+"&idAnio="+anio+"&idMes="+mes;
		NewReport("Informe Financiero Mensual","rpt_inf_financiero.php",params);
	}
	function RepAvanceMes(idproy, anio, mes, ver)
	{
		var params = "idProy="+idproy+"&anio="+anio+"&mes="+mes+"&vs="+ver;
		NewReport("Cumplimiento del Proyecto","rpt_inf_mes_cumpli2.php",params);	
	}
	function RepAvanceAcum(idproy, anio, mes)
	{
		var params = "idProy="+idproy+"&anio="+anio+"&mes="+mes;
		NewReport("Cumplimiento Acumulado del Proyecto","rpt_inf_mes_cumpli_acum2.php",params);	
	}
    </script>
			</p>
