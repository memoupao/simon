...
if (!$idVersion) {
	$idVersion = $ObjSession->MaxVersionProy($idProy);
}
...
if (md5("ajax_edit") == $action) {
    if (md5("ajax_edit") == $action)
        $objFunc->SetSubTitle('V°B° Informe de Supervisión - Editar Registro');
    else
        $objFunc->SetSubTitle('V°B° Informe de Supervisión - Ver Registro');
...
<head>
    ...
    <title>Detalle del Informe de Supervisión</title>
...
?>
    ...
    if ($estadoInf != $HardCode->EstInf_ElaME) {
        $aObsDisFlg = ($ObjSession->PerfilID != $HardCode->SE && $ObjSession->PerfilID != $HardCode->RA && $ObjSession->PerfilID != $HardCode->GP) ? "disabled" : "";
        ?>
                    <tr>
    ...
    $objInf = new BLInformes();
    $mostrar = false;
    $mostrar = $objInf->existeInfSE($idProy, $anio, $idEntregable);
    if ($mostrar) {
        ?>
			...
			<div id="ssTabInforme" class="TabbedPanels">
				<ul class="TabbedPanelsTabGroup">
					...
					<li class="TabbedPanelsTab" tabindex="5" onclick="LoadCalificacion(false);">Aspectos Cualitativos / Calificación</li>
					<li class="TabbedPanelsTab" tabindex="6" onclick="LoadAnexosSE(false);">Anexos</li>
					<li class="TabbedPanelsTab" tabindex="7" onclick="LoadAvanceFinanciero(false);">Avance Financiero</li>
				</ul>
				<div class="TabbedPanelsContentGroup">
								...
								<tbody class="data" bgcolor="#FFFFFF">
									<tr>
										<td align="left" valign="middle">
										    <b>Introducción</b>
											<br/>
											<textarea name="intro" rows="10" id="intro" style="padding: 0px; width: 100%;"><?php echo($row['t30_intro']);?></textarea>
										</td>
									</tr>
									<tr>
										<td align="left" valign="middle">
										    <b>Métodos y Fuentes de Información utilizadas</b><br/>
										    <textarea name="fuentes" rows="10" id="fuentes" style="padding: 0px; width: 100%;"><?php echo($row['t30_fuentes']);?></textarea>
										</td>
									</tr>
								</tbody>
							</table>
					...
					<div class="TabbedPanelsContent">
						<div id="divAvanceFinanciero"></div>
					</div>
				</div>
			</div>
	<?php } else{?>
   ...
   <p>&nbsp;</p>
		...
		function ConfigInfTrim()
		{
            ...
            function Guardar_InformeCab()
        	{
        		...
        		var BodyForm="anio="+$("#anio").val()+"&idEntregable="+$("#idEntregable").val()+"&periodo="+$('#periodo').val()+"&fchPresentacion="+$("#fchPresentacion").val()+"&estadoInf="+$('#estadoInf').val()+"&idProy=<?php echo($idProy);?>&obsGP="+$('#obsGP').val()+"&idVersion="+$("#idVersion").val()+"&iniVisita="+$("#iniVisita").val()+"&terVisita="+$("#terVisita").val()+"&intro="+$("#intro").val()+"&fuentes="+$("#fuentes").val();
        		var sURL = "inf_monext_process.php?action=<?php echo($action);?>";
        	...
        	function informeSuccessCallback(req)
        	{
        	  ...
        	  if(ret=="Exito")
        	  {
        		...
        		dsLista.loadData();
        		btnEditar_Clic('<?php echo($idProy);?>', $('#anio').val(), $('#idEntregable').val());
        	  }
        	  else
        	  {
           ...
           function LoadIndicadoresComponente()
        	{
        		...
        		var BodyForm = "action=<?php echo(md5("lista_ind_comp"));?>&idProy=<?php echo($idProy);?>&idVersion=<?php echo($idVersion);?>&idComp="+comp+"&anio="+$("#anio").val()+"&idEntregable="+$("#idEntregable").val();
        	 	var sURL = "inf_monext_ind_comp.php";
            ...
            function LoadIndicadoresProd()
        	{
        		var BodyForm = "idProy=<?php echo($idProy);?>&idVersion=<?php echo($idVersion);?>&idProd="+ $('#cboprod_ind').val() + "&anio="+$("#anio").val()+"&idEntregable="+$("#idEntregable").val();
        	 	var sURL = "inf_monext_ind_prod.php";
        	...
        	function LoadConclusiones(recargar)
        	{
        		...
        		var BodyForm = "idProy=<?php echo($idProy);?>&anio="+$('#anio').val()+"&idEntregable="+$('#idEntregable').val();
        	 	var sURL = "inf_monext_conclusiones.php";
        	...
        	function LoadCalificacion(recargar)
        	{
        		...
        		var BodyForm = "idProy=<?php echo($idProy);?>&anio="+$('#anio').val()+"&idEntregable="+$('#idEntregable').val();
        	 	var sURL = "inf_monext_califica.php";
        	...
        	function LoadAnexosSE(recargar)
        	{
        	...
        	function LoadAvanceFinanciero(recargar)
        	{
        		if($('#divAvanceFinanciero').html()!="")
        		{
        			if(!recargar){return false;}
        		}

        		var BodyForm = "idProy=<?php echo($idProy);?>&anio="+$('#anio').val()+"&idEntregable="+$('#idEntregable').val();
        	 	var sURL = "inf_monext_avance_financ.php";
        		$('#divAvanceFinanciero').html("<p align='center'><img src='../../../img/indicator.gif' width='16' height='16' /><br>Cargando..<br></p>");
        	 	var req = Spry.Utils.loadURL("POST", sURL, true, SuccessAvanceFinanciero, { postData: BodyForm, headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8" }, errorCallback: onErrorLoad });
        	}

        	function SuccessAvanceFinanciero(req)
        	{
				var respuesta = req.xhRequest.responseText;
				$("#divAvanceFinanciero").html(respuesta);
				return;
        	}
        	...
        	<?php if ($ObjSession->PerfilID == $HardCode->SE || $ObjSession->PerfilID == $HardCode->GP || $ObjSession->PerfilID == $HardCode->RA) { ?>
        	$('#estadoInf').removeAttr('disabled');
        	<?php }
        	...
        	<?php if ($ObjSession->PerfilID == $HardCode->GP) { ?>
                	if( estado == Elaboracion || estado == Correc_ME || estado == Revision) {
