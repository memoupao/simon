...
$objFunc->SetTitle("Informe de Supervisión");
$objFunc->SetSubTitle("Informe de Supervisión");
$objHC = new HardCode();

switch ($objFunc->oSession->PerfilID) {
    case $objHC->SE:
        $res_editar = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección'";
        $res_ver = "'{estado}' == 'Elaboración' || '{estado}' == 'Revisión' || '{estado}' == 'Corrección' || '{estado}' == 'V°B°' || '{estado}' == 'Corrección RA' || '{estado}' == 'Aprobado'";
        $res_elim = "'{estado}' == 'Elaboración'";
        break;
    case $objHC->GP:
        $res_editar = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'Revisión' || '{estado}' == 'Corrección RA'";
        $res_ver = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'V°B°' || '{estado}' == 'Corrección RA' || '{estado}' == 'Aprobado'";
        $res_elim = "'{estado}' == 'NoPuede'";
        break;
    case $objHC->RA:
        $res_editar = "'{estado}' == 'V°B°' || '{estado}' == 'Revisión'";
        $res_ver = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'V°B°' || '{estado}' == 'Revisión' || '{estado}' == 'Corrección RA' || '{estado}' == 'Aprobado'";
        $res_elim = "'{estado}' == 'NoPuede'";
        break;
    case $objHC->FE:
        $res_editar = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'Revisión' || '{estado}' == 'Corrección RA' || '{estado}' == 'Aprobado'";
        $res_ver = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'V°B°' || '{estado}' == 'Revisión' || '{estado}' == 'Corrección RA' || '{estado}' == 'Aprobado'";
        $res_elim = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'V°B°' ||  '{estado}' == 'Revisión' || '{estado}' == 'Corrección RA' || '{estado}' == 'Aprobado'";
        break;
}
...
<head>
    ...
    <script type="text/javascript">
    	...
    	{
    		var xmlurl = "inf_monext_process.xml.php?action=<?php echo(md5("lista_inf_se"));?>&idProy="+idProy+"&idVersion="+idVersion;
								...
								<tr class="RowData" spry:repeat="pvLista" spry:setrow="pvLista" id="{@id}" spry:select="RowSelected">
									    ...
									    <span>
             	                        <?php if($ObjSession->AuthorizedOpcion("EDITAR")) { ?>
                                            <a href="#" <?php if($res_editar){echo('spry:if="'.$res_editar.'"');} ?>>
                                                <img src="../../../img/pencil.gif" width="14" height="14" title="Editar Registro" border="0" onclick="btnEditar_Clic('{t02_anio}','{t02_entregable}'); return false;" />
                                            </a>
                                        <?php } ?>
                                        <?php //if($ObjSession->AuthorizedOpcion("VER")) { ?>
                                            <a href="#" <?php if($res_editar){echo('spry:if="'.$res_ver.'"');}  ?>>
                                                <img src="../../../img/bullet.gif" width="12" height="12" title="Ver Registro" border="0" onclick="btnVer_Clic('{t02_anio}','{t02_entregable}'); return false;" />
                                            </a>
                                        <?php //} ?>
                                        </span>
                                    </td>
									...
									<td align="left">
                                        ...
                                        <a href="#" <?php if($res_elim){echo('spry:if="'.$res_elim.'"');}  ?>>
                                            <img src="../../../img/bt_elimina.gif" width="14" height="14" title="Eliminar Registro" border="0" onclick="Eliminar('{t02_anio}','{t02_entregable}');" />
                                        </a>
                                        <?php } ?>
                                    </td>
								</tr>
				...
				<script language="JavaScript" type="text/javascript">
                    ...
                    function btnNuevo_Clic()
                    {
                	    ...
                	    {alert("Seleccione un Proyecto"); return false;}
                       	...
                       	var url = "inf_monext_edit.php?mode=<?php echo(md5("ajax_new"));?>&idProy="+$('#txtCodProy').val()+"&anio=&idEntregable=&idVersion="+$('#idVersion').val();
                    ...
                    function btnEditar_Clic(anio, idEntregable)
                    {
                      	var url = "inf_monext_edit.php?mode=<?php echo(md5("ajax_edit"));?>&idProy="+$('#txtCodProy').val()+"&anio="+anio+"&idEntregable="+idEntregable+"&idVersion="+$('#idVersion').val();
                      	loadUrlSpry("divContentEdit",url);
                      	$("#divContent").fadeOut("slow");
                      	$("#divContent").css('display', 'none');
                        return;
                    ...
                    function btnVer_Clic(anio, idEntregable)
                    {
                        var url = "inf_monext_edit.php?mode=<?php echo(md5("ajax_view"));?>&idProy="+$('#txtCodProy').val()+"&anio="+anio+"&idEntregable="+idEntregable+"&idVersion="+$('#idVersion').val();
                        loadUrlSpry("divContentEdit",url);
                        $("#divContent").fadeOut("slow");
                        $("#divContent").css('display', 'none');
                        return;
                    ...
                    function Eliminar(anio, idEntregable)
                    {
                        ...
                        if(confirm("¿Está seguro de eliminar el Informe de Supervisión Externa seleccionado?"))
                        {
                        	var BodyForm = "idProy="+$('#txtCodProy').val()+"&anio="+anio+"&idEntregable="+idEntregable;

                        	var sURL = "inf_monext_process.php?action=<?php echo(md5("ajax_del"))?>";
                    ...
                    function ExportarInformeTriTec()
                	{
                    	...
                    	if(row)
                    	{
                    		var arrayControls = new Array();
                    			...
                    			arrayControls[1] = "idVersion=" + (row.vsinf ? row.vsinf : (parseInt(row.t02_anio) + 1));
                    			arrayControls[2] = "idAnio=" + row.t02_anio;
                    			arrayControls[3] = "idTrim=" + (row.t25_trim ? row.t25_trim:row.t02_entregable);
