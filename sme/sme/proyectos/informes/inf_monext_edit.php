...
if (md5("ajax_new") == $action) {
    ...
    $pageMode = 'new';
    $row = $objInf->getSiguienteInfSE($idProy, $idVersion);
    $anio = $row['anio'];
...
if (md5("ajax_edit") == $action || md5("ajax_view") == $action) {
    if (md5("ajax_edit") == $action)
        $objFunc->SetSubTitle('Informe de Supervisión - Editar Registro');
    else
        $objFunc->SetSubTitle('Informe de Supervisión - Ver Registro');
   ...
   <p>&nbsp;</p>
		...
		function ConfigInfTrim()
		{
            ...
            function Guardar_InformeCab()
        	{
        		...
        		var BodyForm="anio="+$("#anio").val()+"&idEntregable="+$("#idEntregable").val()+"&periodo="+$('#periodo').val()+"&fchPresentacion="+$("#fchPresentacion").val()+"&estadoInf="+$('#estadoInf').val()+"&idProy="+$('#txtCodProy').val()+"&obsGP="+obsGP+"&idVersion="+$("#idVersion").val()+"&iniVisita="+$("#iniVisita").val()+"&terVisita="+$("#terVisita").val()+"&intro="+$("#intro").val()+"&fuentes="+$("#fuentes").val();
