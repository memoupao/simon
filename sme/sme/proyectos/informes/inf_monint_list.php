...
$objFunc->SetTitle("Informe de Supervisión Interna");
$objFunc->SetSubTitle("Informe de Supervisión Interna");
$objHC = new HardCode();

switch ($objFunc->oSession->PerfilID) {
    case $objHC->GP:
        $res_editar = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección'";
        $res_ver = "'{estado}' == 'Elaboración' || '{estado}' == 'Revisión' || '{estado}' == 'Corrección' || '{estado}' == 'Aprobado'";
        $res_elim = "'{estado}' == 'Elaboración'";
        break;
    case $objHC->RA:
        $res_editar = "'{estado}' == 'Revisión'";
        $res_ver = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'Revisión' || '{estado}' == 'Aprobado'";
        $res_elim = "'{estado}' == 'NoPuede'";
        break;
    case $objHC->FE:
        $res_editar = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'Revisión' || '{estado}' == 'Aprobado'";
        $res_ver = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'Revisión' || '{estado}' == 'Aprobado'";
        $res_elim = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'Revisión' || '{estado}' == 'Aprobado'";
        break;
}
...
<head>
    ...
    <script type="text/javascript">
    	...
    	{
    		var xmlurl = "inf_monint_process.xml.php?action=<?php echo(md5("lista_inf_si"));?>&idProy="+idProy+"&idVersion="+idVersion;
				...
				<script language="JavaScript" type="text/javascript">
                    ...
                    function btnNuevo_Clic()
                    {
                	    ...
                	    {alert("Seleccione un Proyecto"); return false;}
                       	...
                       	var url = "inf_monint_edit.php?mode=<?php echo(md5("ajax_new"));?>&idProy="+$('#txtCodProy').val()+"&anio=&idEntregable=&idVersion="+$('#idVersion').val();
                    ...
                    {
                      	var url = "inf_monint_edit.php?mode=<?php echo(md5("ajax_edit"));?>&idProy="+$('#txtCodProy').val()+"&anio="+anio+"&idEntregable="+idEntregable+"&idVersion="+$('#idVersion').val();
                      	loadUrlSpry("divContentEdit",url);
                      	$("#divContent").fadeOut("slow");
                      	$("#divContent").css('display', 'none');
                        return;
                    ...
                    {
                        var url = "inf_monint_edit.php?mode=<?php echo(md5("ajax_view"));?>&idProy="+$('#txtCodProy').val()+"&anio="+anio+"&idEntregable="+idEntregable+"&idVersion="+$('#idVersion').val();
                        loadUrlSpry("divContentEdit",url);
                        $("#divContent").fadeOut("slow");
                        $("#divContent").css('display', 'none');
                        return;
                    ...
                    function Eliminar(anio, idEntregable)
                    {
                        ...
                        if(confirm("¿Está seguro de eliminar el Informe de Supervisión Interna seleccionado?"))
                        {
                        	var BodyForm = "idProy="+$('#txtCodProy').val()+"&anio="+anio+"&idEntregable="+idEntregable;

                        	var sURL = "inf_monint_process.php?action=<?php echo(md5("ajax_del"))?>";
