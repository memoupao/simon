...
<?php
$objFunc->SetTitle("Informe Trimestral Técnico - Proyectos");
$objFunc->SetSubTitle("Informe Técnico Trimestral ");
?>
...
switch ($objFunc->oSession->PerfilID) {
    case $objHC->Ejec:
        ...
        $res_ver = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'VB'";
    ...
    case $objHC->MF:
        ...
        break;
    case $objHC->Admin:
        $res_editar = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'Revision'";
        $res_ver = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'VB' || '{estado}' == 'Revision'";
        $res_elim = "'{estado}' == 'Elaboración' || '{estado}' == 'Corrección' || '{estado}' == 'VB' ||  '{estado}' == 'Revision'";
        break;
}
...
function Loadproyectos()
{
	...
	{
		var xmlurl = "inf_trim_process.xml.php?action=<?php echo(md5("lista_inf_trim"));?>&idProy=" + idProy ;
		if(dsLista!=null)
		{
...
?>
 ...
 <?php } ?>
						...
						<table class="grid-table grid-width">
							<thead>
								<tr>
									...
									<th width="35" align="center" spry:sort="trim">Trim</th>
									<th width="119" align="center" spry:sort="periodo">Periodo
										Referencia</th>
            ...
            <a href="#"
											<?php if($res_editar){echo('spry:if="'.$res_editar.'"');}  ?>><img
												...
												onclick="btnEditar_Clic('{t25_anio}','{t25_trim}','{vsinf}'); return false;" /></a>
            <?php } ?>
            <?php //if($ObjSession->AuthorizedOpcion("VER")) { ?>
            <a href="#"
											<?php if($res_editar){echo('spry:if="'.$res_ver.'"');}  ?>><img
												src="../../../img/bullet.gif" width="12" height="12"
												title="Ver Registro" border="0"
												onclick="btnVer_Clic('{t25_anio}','{t25_trim}','{vsinf}'); return false;" /></a>
            <?php //} ?>
          </span></td>
									<td align="center" nowrap="nowrap">{anio}</td>
									<td align="center" nowrap="nowrap">{trim}</td>
        ...
        <a href="#"
										<?php if($res_elim){echo('spry:if="'.$res_elim.'"');}  ?>><img
											...
											onclick="Eliminar('{t25_anio}','{t25_trim}','{vsinf}');" /></a>
        <?php } ?>
        </td>
								</tr>
							</tbody>
 ...
 function btnNuevo_Clic()
  {
	...
	var url = "inf_trim_edit.php?mode=<?php echo(md5("ajax_new"));?>&idProy="+$('#txtCodProy').val()+"&anio=&trim=";
	loadUrlSpry("divContentEdit",url);
	return;
  }

 function btnVer_Clic(anio, trim, vs)
  {
	Editar_Clic_view(anio, trim, vs, "<?php echo(md5("ver"));?>") ;
  }

 function btnEditar_Clic(anio, trim, vs)
  {
	Editar_Clic_view(anio, trim, vs, "") ;
  }

 function Editar_Clic_view(anio, trim, vs, action)
  {
	var url = "inf_trim_edit.php?mode=<?php echo(md5("ajax_edit"));?>&idProy="+$('#txtCodProy').val()+"&idanio="+anio+"&idtrim="+trim+"&idversion="+vs+"&accion="+action;;
	loadUrlSpry("divContentEdit",url);
	$("#divContent").fadeOut("slow");
	$("#divContent").css('display', 'none');
 ...
 function CancelEdit ()
 {
  ...
  function Eliminar(anio, trim, vs)
  {
	...
	{
		var BodyForm = "t02_cod_proy="+$('#txtCodProy').val()+"&t25_anio="+anio+"&t25_trim="+trim+"&t25_ver_inf="+vs;

		var sURL = "inf_trim_process.php?action=<?php echo(md5("ajax_del"))?>";
