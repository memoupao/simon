...
if (md5("ajax_new") == $action) {
    ...
    $row = 0;
    $row = $objInf->InformeFinancierolUltimo($idProy);
    $idAnio = $row['new_anio'];
    $idMes = $row['new_mes'];

    /* list ($idAnio, $idMes) = $objInf->GetStartingYearMoth($idProy, $idAnio, $idMes);
    list ($aPerNum, $aPerTxt) = $objInf->GetStartingPeriod($idProy, $idAnio, $idMes);

    $row['t40_periodo'] = $aPerNum ? $aPerNum : $row['t40_periodo']; */
}
...
<script src="../../../jquery.ui-1.5.2/jquery.maskedinput.js"
		...
		<div id="toolbar" style="height: 4px;" class="BackColor">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td width="10%"><button class="Button"
          ...
          <?php } ?>
					...
					<td width="15%">
						<!--<button class="Button" onclick="btnImportar_Clic(); return false;" disabled="disabled" value="Cancelar"> Importar Gastos</button>-->
			 <?php if($monitor_fe){?>
   ...
   function EnviarInformeA()
	...
	function informeSuccessCallback	(req)
	{
	  ...
	  if(ret=="Exito")
	  {
		...
		btnEditar_Clic($('#cboanio').val(), $('#cbomes').val());
	  }
	  else
	  {alert(respuesta);}
