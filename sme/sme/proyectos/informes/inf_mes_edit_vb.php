...
?>
		...
		<div id="toolbar" style="height: 4px;" class="BackColor">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
				    <td>Proyecto: <?php echo($idProy);?></td>
	...
	var BodyForm="cboanio="+anio+"&t20_anio="+$("#t20_anio").val()+"&t20_ver_inf="+$("#t20_ver_inf").val()+"&cbomes="+mes+"&t20_mes="+$("#t20_mes").val()+"&t20_periodo="+per+"&t20_fch_pre="+$("#t20_fch_pre").val()+"&t20_estado="+est+"&t02_cod_proy=<?php echo($idProy);?>&vb_se="+checkbox;
	var sURL = "inf_mes_process.php?action=<?php echo(md5("ajax_edit_vb"));?>";
	...
	function informeSuccessCallback	(req)
	{
	  ...
	  if(ret=="Exito")
	  {
		...
		vs = vs.replace(ret,"");
		btnEditar_Clic('<?php echo($idProy);?>', $('#cboanio').val(), $('#cbomes').val(), vs, '');
	  }
	  else
	  {alert(respuesta);}
...
if (md5("ajax_edit") == $view) {
	...
	<?php if($perfil == $objHC->SE)  { ?>
    	if(estado==Revision)
    	  {
    		$('#t20_estado option[value="'+Correccion+'"]').removeAttr('disabled');
    	  }
    <?php } ?>
	...
	<?php $ObjSession->AuthorizedPage(); ?>
		...
		if(confirm("Esta seguro de que desea enviar el informe a estado en Corrección?"))
		{
			$('.wrapper_wait').show();
	...
	function Enviar_CorrecionCallback(req)
	{
	  ...
	  if(ret=="Éxito")
	  {
		...
		var mes = $('#cbomes').val();
		var ver = $('#t20_ver_inf').val();
		btnEditar_Clic('<?php echo($idProy);?>', anio, mes, ver, '<?php echo(md5("ver"));?>', '');
