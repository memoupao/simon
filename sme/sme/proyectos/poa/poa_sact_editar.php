...
$t02_cod_proy = $Function->__POST('t02_cod_proy');
$t02_version = $Function->__POST('t02_version');
$t08_cod_comp = $Function->__POST('t08_cod_comp');
$t09_cod_act = $Function->__POST('t09_cod_act');

$vista = $Function->__GET('view');
...
if ($proc == "save") {
    ...
    if ($ReturnPage) {
        ...
        $script .= "parent.btnSuccessSubAct('" . $t02_cod_proy . "'," . $t02_version . "," . $t08_cod_comp . "," . $t09_cod_act . "); \n";
...
if ($proc == md5("del")) {
    ...
    if ($ReturnPage) {
        ...
        $t02_cod_proy = $Function->__GET('t02_cod_proy');
        $t02_version = $Function->__GET('t02_version');
        $t08_cod_comp = $Function->__GET('t08_cod_comp');
        $t09_cod_act = $Function->__GET('t09_cod_act');
...
$urlSave = "poa_sact_editar.php?idProy=$idProy&idVersion=$idVersion&idComp=$idComp&idActiv=$idActiv";
...
<body>
<?php if($ObjSession->MaxVersionProy($idProy) > 1 && ($vista=='edit')) {$lsReadOnly = 'readonly="readonly"' ; } else { $lsReadOnly ='';} ?>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td width="9%"><button class="Button" value="Cancelar"
							onclick="return Cancelar(<?php echo("'".$idProy."',".$idVersion.",".$idComp.",".$idActiv);?>);">
							Cancelar</button></td>
				</tr>
			</table>
		</div>
	...
	<script language="javascript" type="text/javascript">
	  function Cancelar(idProy,idVersion,idComp,idActiv)
	  {				
		 parent.btnSuccessSubAct(idProy,idVersion,idComp,idActiv);
		 return false;
	  }
