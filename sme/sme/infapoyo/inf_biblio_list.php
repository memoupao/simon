...
<body class="oneColElsCtrHdr">
    ...
    <?php include("../../includes/subtitle.php");?>
		...
		<div id="mainContent">
            ...
            // if($ObjSession->PerfilID == $HC->Admin || $ObjSession->PerfilID == $HC->EVA || $ObjSession->PerfilID == $HC->MF || $ObjSession->PerfilID == $HC->MT || $ObjSession->PerfilID == $HC->CMT || $ObjSession->PerfilID == $HC->CMF || $ObjSession->PerfilID == $HC->SE)
            // --------------------------------------------------<
            ?>
             <?php if($ObjSession->PerfilID == $HC->GP || $ObjSession->PerfilID == $HC->RA) { ?>
     ...
     <?php if($action=md5("mostrar_libro") )  { ?>
        ...
        while ($row = mysql_fetch_assoc($iRs)) {
            ...
            <tr>
												...
												<tr>
													<td valign="top"><strong>Edición:</strong></td>
													<td><?php echo( $row['edicion']);?></td>
												</tr>
												<tr>
													<td valign="top"><strong>Sector:</strong></td>

													<td><?php echo( $row['descrip']);?></td>
												</tr>
												<tr>
													<td valign="top"><strong>Palabras Clave:</strong></td>
            ...
            <tr>
													<td valign="top"><strong>Archivo:</strong></td>
    ...
    ?>
                ...
                ?>
								...
								<tr>
									...
									<td colspan="3">Solo puede subir, los archivos con extensión</td>
								</tr>
								<tr>
									<td align="left" valign="top">&nbsp;</td>
									<td colspan="3">(pdf, doc, docx, xls, xlsx, ppt, pptx, pps,
										jpg, png, tiff)</td>
								</tr>
								<tr>
									<td align="left" valign="top">&nbsp;</td>
									...
									<td height="36"><span id="toolbar"
										style="width: 70px; height: 8px;"> &nbsp;&nbsp;&nbsp;
											<button class="Button"
												...
												Guardar</button>
									</span> <iframe id="ifrmUploadFile" name="ifrmUploadFile"
...
function GuardarDocumento()
{
 ...
 if(titulo=='' || titulo==null){alert("Ingrese el título"); $('#titulo').focus(); return false;}
 if(autor=='' || autor==null){alert("Ingrese el autor"); $('#autor').focus(); return false;}

     ext_permitidas = new Array(".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx", ".pps", ".jpg", ".png", ".tiff", ".pdf");

	 extension = (arch.substring(arch.lastIndexOf("."))).toLowerCase();
	 var x=0;
	 ...
	 if(x==0)
	 	{
		alert("El tipo de archivo '"+extension+"' no es permitido");
	   	return false;
		}
