...
class Block_Frame_Decorator extends Frame_Decorator
{
    ...
    protected static $_initial_line_state = array(
        "frames" => array(),
        "wc" => 0,
        "y" => null,
        "w" => 0,
    ...
    function add_frame_to_line(Frame $frame)
    {
        ...
        if (in_array($style->position, array(
            "absolute",
            "fixed"
        ...
        if ($w == 0)
            ...
            // Debugging code:
            /*
         * pre_r("\n<h3>Adding frame to line:</h3>"); // pre_r("Me: " . $this->get_node()->nodeName . " (" . spl_object_hash($this->get_node()) . ")"); // pre_r("Node: " . $frame->get_node()->nodeName . " (" . spl_object_hash($frame->get_node()) . ")"); if ( $frame->get_node()->nodeName === "#text" ) pre_r('"'.$frame->get_node()->nodeValue.'"'); pre_r("Line width: " . $this->_lines[$this->_cl]["w"]); pre_r("Frame: " . get_class($frame)); pre_r("Frame width: " . $w); pre_r("Frame height: " . $frame->get_margin_height()); pre_r("Containing block width: " . $this->get_containing_block("w"));
         */
            // End debugging
