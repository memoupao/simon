...
class styleHTML
{
     ...
     *            array	tableau de style
     * @param
     *            array	tableau initialisant des styles
     * @return null
     */
    function analyse($balise, &$param, $heritage = null)
    {
        ...
        foreach ($styles as $nom => $val) {
            switch ($nom) {
                ...
                case 'border-radius':
                    ...
                    if (count($val_h) < 1 || count($val_h) > 4)
                        break;
                    if (! isset($val_h[1]))
                        $val_h[1] = $val_h[0];
                    if (! isset($val_h[2]))
                    ...
                    if (isset($val[1])) {
                        ...
                        if (count($val_v) < 1 || count($val_v) > 4)
                            break;
                        if (! isset($val_v[1]))
                            $val_v[1] = $val_v[0];
                        if (! isset($val_v[2]))
        ...
        if ($no_width && in_array($balise, array(
            'div',
            'fieldset'
        )) && $this->value['position'] != 'absolute') {
     ...
     *
     * @return array() des propriétés CSS
     */
    function getFromCSS()
    {
     ...
     *
     * @param
     *            string		clef CSS à analyser
     * @param
     *            array()		tableau des styles direct, et ceux des parents
     * @param
     *            string		prochaine etape
     * @return boolean autorisée ou non
     */
    function getReccursiveStyle($key, $lst, $next = null)
    {
        ...
        if ($next !== null) {
            if ($next)
                $key = trim(substr($key, 0, - strlen($next))); // on elève cette etape
            unset($lst[0]);
            if (! count($lst))
        ...
        foreach ($lst[0] as $nom) {
            ...
            if (substr($key, - strlen(' ' . $nom)) == ' ' . $nom && $this->getReccursiveStyle($key, $lst, $nom))
                return true; // si la clef est la fin, on analyse ce qui précède
        }
     ...
     *
     * @param
     *            string		propriété border
     * @return array() décodée
     */
    ...
    function ConvertBackgroundColor($val)
    {
        ...
        if ($val == 'transparent')
            ...
            return $this->ConvertToColor($val, $res);
    }
    ...
    function ConvertBackgroundImage($val)
    {
        if ($val == 'none')
            ...
            if (preg_match('/^url\(([^)]*)\)$/isU', $val, $match))
                return $match[1];
            else
                return null;
    }
                ...
                if ($val[0] == 'right')
                    ...
                    if ($val[0] == 'top')
                        ...
                        if ($val[0] == 'bottom')
                            ...
                            if (preg_match('/^[-]?[0-9\.]+%$/isU', $val[0]))
                                $x = $val[0];
                            else 
                                if ($this->ConvertToMM($val[0]))
                ...
                if ($val[1] == 'top')
                    ...
                    if ($val[1] == 'center')
                        ...
                        if ($val[1] == 'bottom')
                            ...
                            if (preg_match('/^[-]?[0-9\.]+%$/isU', $val[1]))
                                $y = $val[1];
                            else 
                                if ($this->ConvertToMM($val[1]))
     ...
     *
     * @param
     *            string			longueur, avec unité, à convertir
     * @param
     *            float			longueur du parent
     * @return float exprimée en mm
     */
    function ConvertToMM($val, $old = 0.)
    {
        ...
        if (preg_match('/^[0-9\.\-]+px$/isU', $val))
            ...
            if (preg_match('/^[0-9\.\-]+pt$/isU', $val))
                $val = 25.4 / 72. * str_replace('pt', '', $val);
            else 
                if (preg_match('/^[0-9\.\-]+in$/isU', $val))
                    $val = 25.4 * str_replace('in', '', $val);
                else 
                    if (preg_match('/^[0-9\.\-]+mm$/isU', $val))
