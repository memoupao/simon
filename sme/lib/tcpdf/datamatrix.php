...
class Datamatrix
{
    ...
    protected $chset = array(
		...
		'TXT'//
        null => array( // Basic set for TEXT ---------------------------------------------------------------------------
			'S1'// Basic set for TEXT ---------------------------------------------------------------------------
            null => 0x00,
            'S2' => 0x01,
            'S3' => 0x02,
            0x20 => 0x03,
		...
		'S3C'//
        null => array( // Shift 3 set for C40 --------------------------------------------------------------------------
			0x60// Shift 3 set for C40 --------------------------------------------------------------------------
            null => 0x00,
            0x61 => 0x01,
            0x62 => 0x02,
            0x63 => 0x03,
	 ...
	 */
	// -----------------------------------------------------------------------------
    nullpublic function __construct($code)
    {
        $barcode_array = array();
        ...
        // get data codewords
		$cw// get data codewords
        null = $this->getHighLevelEncoding($code);
        // number of data codewords
		$nd// number of data codewords
        null = count($cw);
        // check size
		if// check size
        null ($nd > 1558) {
		...
		foreach// get minimum required matrix size.
        null ($this->symbattr as $params) {
            if ($params[11] >= $nd) {
                break;
            }
        }
        if ($params[11] < $nd) {
            ...
            // add padding
			if// add padding
            null ($this->last_enc == ENC_EDF) {
                // switch to ASCII encoding
				$cw// switch to ASCII encoding
                null[] = 124;
                ++ $nd;
            ...
            if ($params[11] > $nd) {
                ...
                ++ $nd;
                // add remaining pads
				for// add remaining pads
                null ($i = $nd; $i <= $params[11]; ++ $i) {
                    $cw[] = $this->get253StateCodeword(129, $i);
                }
            }
        }
        // add error correction codewords
		$cw// add error correction codewords
        null = $this->getErrorCorrection($cw, $params[13], $params[14], $params[15]);
        // initialize empty arrays
		$grid// initialize empty arrays
        null = array_fill(0, ($params[2] * $params[3]), 0);
        // get placement map
		$places// get placement map
        null = $this->getPlacemetMap($params[2], $params[3]);
        // fill the grid with data
		$grid// fill the grid with data
        null = array();
        $i = 0;
        // region data row max index
		$rdri// region data row max index
        null = ($params[4] - 1);
        // region data column max index
		$rdci// region data column max index
        null = ($params[5] - 1);
        // for each vertical region
		for// for each vertical region
        null ($vr = 0; $vr < $params[9]; ++ $vr) {
            // for each row on region
			for// for each row on region
            null ($r = 0; $r < $params[4]; ++ $r) {
                // get row
				$row// get row
                null = (($vr * $params[4]) + $r);
                // for each horizontal region
				for// for each horizontal region
                null ($hr = 0; $hr < $params[8]; ++ $hr) {
                    // for each column on region
					for// for each column on region
                    null ($c = 0; $c < $params[5]; ++ $c) {
                        // get column
						$col// get column
                        null = (($hr * $params[5]) + $c);
                        // braw bits by case
						if// braw bits by case
                        null ($r == 0) {
                            // top finder pattern
							if// top finder pattern
                            null ($c % 2) {
                                $grid[$row][$col] = 0;
                            } else {
                                $grid[$row][$col] = 1;
							...
							$grid// bottom finder pattern
                            null[$row][$col] = 1;
                        } elseif ($c == 0) {
                            // left finder pattern
							$grid// left finder pattern
                            null[$row][$col] = 1;
                        } elseif ($c == $rdci) {
                            // right finder pattern
							if// right finder pattern
                            null ($r % 2) {
                                $grid[$row][$col] = 1;
                            } else {
                                $grid[$row][$col] = 0;
                            ...
                            null ($places[$i] < 2) {
                                ...
                                // codeword ID
								$cw_id// codeword ID
                                null = (floor($places[$i] / 10) - 1);
                                // codeword BIT mask
								$cw_bit// codeword BIT mask
                                null = pow(2, (8 - ($places[$i] % 10)));
    ...
    protected function getErrorCorrection($wd, $nb, $nd, $nc, $gf = 256, $pp = 301)
    {
		...
		$log// generate the log ($log) and antilog ($alog) tables
        null[0] = 0;
        $alog[0] = 1;
        for ($i = 1; $i < $gf; ++ $i) {
            $alog[$i] = ($alog[($i - 1)] * 2);
		...
		$c// generate the polynomial coefficients (c)
        null = array_fill(0, ($nc + 1), 0);
        $c[0] = 1;
        for ($i = 1; $i <= $nc; ++ $i) {
            $c[$i] = $c[($i - 1)];
            for ($j = ($i - 1); $j >= 1; -- $j) {
		...
		$num_wd// total number of data codewords
        null = ($nb * $nd);
        // total number of error codewords
		$num_we// total number of error codewords
        null = ($nb * $nc);
        // for each block
		for// for each block
        null ($b = 0; $b < $nb; ++ $b) {
            // create interleaved data block
			$block// create interleaved data block
            null = array();
            for ($n = $b; $n < $num_wd; $n += $nb) {
                $block[] = $wd[$n];
            }
            // initialize error codewords
			$we// initialize error codewords
            null = array_fill(0, ($nc + 1), 0);
            // calculate error correction codewords for this block
			for// calculate error correction codewords for this block
            null ($i = 0; $i < $nd; ++ $i) {
                $k = ($we[0] ^ $block[$i]);
                for ($j = 0; $j < $nc; ++ $j) {
			...
			$j// add error codewords at the end of data codewords
            null = 0;
    ...
    protected function isCharMode($chr, $mode)
    {
        ...
        switch ($mode) {
            case ENC_ASCII:
				...
				$status// ASCII character 0 to 127
                    null = (($chr >= 0) and ($chr <= 127));
                    break;
                }
            case ENC_C40:
                { // Upper-case alphanumeric
				$status// Upper-case alphanumeric
                    null = (($chr == 32) or (($chr >= 48) and ($chr <= 57)) or (($chr >= 65) and ($chr <= 90)));
                    break;
                }
            case ENC_TXT:
                { // Lower-case alphanumeric
				$status// Lower-case alphanumeric
                    null = (($chr == 32) or (($chr >= 48) and ($chr <= 57)) or (($chr >= 97) and ($chr <= 122)));
                    break;
                }
            case ENC_X12:
                { // ANSI X12
				$status// ANSI X12
                    null = (($chr == 13) or ($chr == 42) or ($chr == 62));
                    break;
                }
            case ENC_EDF:
                { // ASCII character 32 to 94
				$status// ASCII character 32 to 94
                    null = (($chr >= 32) and ($chr <= 94));
                    break;
                }
            case ENC_BASE256:
                { // Function character (FNC1, Structured Append, Reader Program, or Code Page)
				$status// Function character (FNC1, Structured Append, Reader Program, or Code Page)
                    null = (($chr == 232) or ($chr == 233) or ($chr == 234) or ($chr == 241));
                    break;
                }
            case ENC_ASCII_EXT:
                { // ASCII character 128 to 255
				$status// ASCII character 128 to 255
                    null = (($chr >= 128) and ($chr <= 255));
                    break;
                }
            case ENC_ASCII_NUM:
                { // ASCII digits
				$status// ASCII digits
                    null = (($chr >= 48) and ($chr <= 57));
                    break;
                }
        }
        return $status;
    ...
    protected function lookAheadTest($data, $pos, $mode)
    {
        ...
        while (true) {
            // STEP K
			if// STEP K
            null (($pos + $charscount) == $data_length) {
            ...
            // get char
			$chr// get char
            null = ord($data{($pos + $charscount)});
            $charscount ++;
            // STEP L
			if// STEP L
            null ($this->isCharMode($chr, ENC_ASCII_NUM)) {
                $numch[ENC_ASCII] += (1 / 2);
            ...
            // STEP M
			if// STEP M
            null ($this->isCharMode($chr, ENC_C40)) {
                $numch[ENC_C40] += (2 / 3);
            } elseif ($this->isCharMode($chr, ENC_ASCII_EXT)) {
                $numch[ENC_C40] += (8 / 3);
            } else {
            ...
            // STEP N
			if// STEP N
            null ($this->isCharMode($chr, ENC_TXT)) {
                $numch[ENC_TXT] += (2 / 3);
            } elseif ($this->isCharMode($chr, ENC_ASCII_EXT)) {
                $numch[ENC_TXT] += (8 / 3);
            } else {
            ...
            // STEP O
			if// STEP O
            null ($this->isCharMode($chr, ENC_X12) or $this->isCharMode($chr, ENC_C40)) {
                $numch[ENC_X12] += (2 / 3);
            } elseif ($this->isCharMode($chr, ENC_ASCII_EXT)) {
                $numch[ENC_X12] += (13 / 3);
            } else {
            ...
            // STEP P
			if// STEP P
            null ($this->isCharMode($chr, ENC_EDF)) {
                $numch[ENC_EDF] += (3 / 4);
            } elseif ($this->isCharMode($chr, ENC_ASCII_EXT)) {
                $numch[ENC_EDF] += (17 / 4);
            } else {
            ...
            // STEP Q
			if// STEP Q
            null ($this->isCharMode($chr, ENC_BASE256)) {
                $numch[ENC_BASE256] += 4;
            } else {
                $numch[ENC_BASE256] += 1;
            }
    ...
    protected function getSwitchEncodingCodeword($mode)
    {
        switch ($mode) {
            case ENC_ASCII:
                { // ASCII character 0 to 127
				$cw// ASCII character 0 to 127
                    null = 254;
                    break;
                }
            case ENC_C40:
                { // Upper-case alphanumeric
				$cw// Upper-case alphanumeric
                    null = 230;
                    break;
                }
            case ENC_TXT:
                { // Lower-case alphanumeric
				$cw// Lower-case alphanumeric
                    null = 239;
                    break;
                }
            case ENC_X12:
            ...
            case ENC_EDF:
                { // ASCII character 32 to 94
				$cw// ASCII character 32 to 94
                    null = 240;
                    break;
                }
            case ENC_BASE256:
                { // Function character (FNC1, Structured Append, Reader Program, or Code Page)
				$cw// Function character (FNC1, Structured Append, Reader Program, or Code Page)
                    null = 231;
                    break;
                }
        }
    ...
    protected function getHighLevelEncoding($data)
    {
		...
		$enc// STEP A. Start in ASCII encodation.
        null = ENC_ASCII; // current encoding mode
		$pos// current encoding mode
        null = 0; // current position
		$cw// current position
        null = array(); // array of codewords to be returned
		$cw_num// array of codewords to be returned
        null = 0; // number of data codewords
		$data_lenght// number of data codewords
        null = strlen($data); // number of chars
		while// number of chars
        null ($pos < $data_lenght) {
            switch ($enc) {
                case ENC_ASCII:
                    { // STEP B. While in ASCII encodation
					if// STEP B. While in ASCII encodation
                        null (($data_lenght > 1) and ($pos < ($data_lenght - 1)) and ($this->isCharMode(ord($data{($pos)}), ENC_ASCII_NUM) and $this->isCharMode(ord($data{($pos + 1)}), ENC_ASCII_NUM))) {
                            // 1. If the next data sequence is at least 2 consecutive digits, encode the next two digits as a double digit in ASCII mode.
						$cw// 1. If the next data sequence is at least 2 consecutive digits, encode the next two digits as a double digit in ASCII mode.
                            null[] = (intval(substr($data, $pos, 2)) + 130);
                            ++ $cw_num;
                            $pos += 2;
                        } else {
                            // 2. If the look-ahead test (starting at step J) indicates another mode, switch to that mode.
						$newenc// 2. If the look-ahead test (starting at step J) indicates another mode, switch to that mode.
                            null = $this->lookAheadTest($data, $pos, $enc);
                            if ($newenc != $enc) {
                                // switch to new encoding
							$enc// switch to new encoding
                                null = $newenc;
							...
							$chr// get new byte
								...
								$cw// 3. If the next data character is extended ASCII (greater than 127) encode it in ASCII mode first using the Upper Shift (value 235) character.
                                    null[] = 235;
                                    $cw[] = ($chr - 127);
                                    $cw_num += 2;
                                } else {
                                    // 4. Otherwise process the next data character in ASCII encodation.
								$cw// 4. Otherwise process the next data character in ASCII encodation.
                                    null[] = ($chr + 1);
                                    ++ $cw_num;
                                }
                            }
				...
				case// Upper-case alphanumeric
                null ENC_TXT: // Lower-case alphanumeric
				case// Lower-case alphanumeric
                null ENC_X12:
                    { // ANSI X12
					$temp_cw// ANSI X12
                        null = array();
                        ...
                        $epos = $pos;
                        // get charset ID
					$set_id// get charset ID
                        null = $this->chset_id[$enc];
                        // get basic charset for current encoding
					$charset// get basic charset for current encoding
                        null = $this->chset[$set_id];
                        do {
                            // 2. process the next character in C40 encodation.
						$chr// 2. process the next character in C40 encodation.
                            null = ord($data{($epos)});
                            ++ $epos;
                            // check for extended character
						if// check for extended character
                            null ($chr & 0x80) {
                                if ($enc == ENC_X12) {
                                    return false;
                                }
							...
							$newenc// 1. If the C40 encoding is at the point of starting a new double symbol character and if the look-ahead test (starting at step J) indicates another mode, switch to that mode.
                                null = $this->lookAheadTest($data, $pos, $enc);
                                if ($newenc != $enc) {
                                    $enc = $newenc;
                                    $cw[] = $this->getSwitchEncodingCodeword($enc);
					...
					if// process last data (if any)
                        null ($p > 0) {
                            // get remaining number of data symbols
						$cwr// get remaining number of data symbols
                            null = ($this->getMaxDataCodewords($cw_num + 2) - $cw_num);
                            if (($cwr == 1) and ($p == 1)) {
                                // d. If one symbol character remains and one C40 value (data character) remains to be encoded
							$c1// d. If one symbol character remains and one C40 value (data character) remains to be encoded
                                null = array_shift($temp_cw);
                                -- $p;
                                $cw[] = ($c1 + 1);
                                ++ $cw_num;
                            } elseif (($cwr == 2) and ($p == 1)) {
                                // c. If two symbol characters remain and only one C40 value (data character) remains to be encoded
							$c1// c. If two symbol characters remain and only one C40 value (data character) remains to be encoded
                                null = array_shift($temp_cw);
                                -- $p;
                                $cw[] = 254;
                                $cw[] = ($c1 + 1);
							...
							$c1// b. If two symbol characters remain and two C40 values remain to be encoded
                                null = array_shift($temp_cw);
                                $c2 = array_shift($temp_cw);
                                $p -= 2;
                                $tmp = ((1600 * $c1) + (40 * $c2) + 1);
                ...
                case ENC_EDF:
					...
					$temp_cw// initialize temporary array with 0 lenght
                        null = array();
                        $epos = $pos;
                        $field_lenght = 0;
                        while ($epos < $data_lenght) {
                            // 2. process the next character in EDIFACT encodation.
						$chr// 2. process the next character in EDIFACT encodation.
                            null = ord($data{($epos)});
                            ++ $epos;
                            $temp_cw[] = $chr;
                            ++ $field_lenght;
                            if (($field_lenght == 4) or ($epos == $data_lenght)) {
								...
								$temp_cw// set unlatch character
                                    null[] = 0x1f;
                                    ++ $field_lenght;
                                    $enc = ENC_ASCII;
                                    // fill empty characters
								for// fill empty characters
                                    null ($i = $field_lenght; $i < 4; ++ $i) {
                                        $temp_cw[] = 0;
                                    }
                                }
                                // encodes four data characters in three codewords
							$cw// encodes four data characters in three codewords
                                null[] = (($temp_cw[0] & 0x3F) << 2) + (($temp_cw[1] & 0x30) >> 4);
						...
						if// 1. If the EDIFACT encoding is at the point of starting a new triple symbol character and if the look-ahead test (starting at step J) indicates another mode, switch to that mode.
                            null ($field_lenght == 0) {
                                // get remaining number of data symbols
							$cwr// get remaining number of data symbols
                                null = ($this->getMaxDataCodewords($cw_num + 2) - $cw_num);
								...
								$enc// return to ascii without unlatch
                                    null = ENC_ASCII;
                                ...
                                null else {
									...
									$enc// 1. If the look-ahead test (starting at step J) indicates another mode, switch to that mode.
                                        null = $newenc;
                                        $cw[] = $this->getSwitchEncodingCodeword($enc);
                                        ++ $cw_num;
                                        break; // exit from EDIFACT mode
                ...
                case ENC_BASE256:
					...
					$temp_cw// initialize temporary array with 0 lenght
                        ...
                        while (($pos < $data_lenght) and ($field_lenght <= 1555)) {
							...
							$chr// 2. Otherwise, process the next character in Base 256 encodation.
                                null = ord($data{($pos)});
                                ++ $pos;
                                $temp_cw[] = $chr;
                                ++ $field_lenght;
					...
					if// set field lenght
                        ...
                        if (! empty($temp_cw)) {
                            // add B256 field
						foreach// add B256 field
                            null ($temp_cw as $p => $cht) {
		...
		}// end of switch enc // end of while
          // set last used encoding
		$this// set last used encoding
        null->last_enc = $enc;
        return $cw;
    }
    ...
    protected function getPlacemetMap($nrow, $ncol)
    {
		...
		$marr// initialize array with zeros
        null = array_fill(0, ($nrow * $ncol), 0);
        // set starting values
		$chr// set starting values
        null = 1;
        $row = 4;
        $col = 0;
        do {
            // repeatedly first check for one of the special corner cases, then
			if// repeatedly first check for one of the special corner cases, then
            null (($row == $nrow) and ($col == 0)) {
			...
			do// sweep upward diagonally, inserting successive characters,
            null {
			...
			do// & then sweep downward diagonally, inserting successive characters,...
            null {
        ...
        null while (($row < $nrow) or ($col < $ncol));
        // lastly, if the lower righthand corner is untouched, fill in fixed pattern
		if// lastly, if the lower righthand corner is untouched, fill in fixed pattern
        null (! $marr[(($nrow * $ncol) - 1)]) {
            $marr[(($nrow * $ncol) - 1)] = 1;
