...
class PDF417
{
    ...
    nullprotected $rsfactors = array(
        array( // ECL 0 (2 factors) -------------------------------------------------------------------------------
			0x01b// ECL 0 (2 factors) -------------------------------------------------------------------------------
            null,
            0x395
        ), //   2
		array// 2 // ECL 1 (4 factors) -------------------------------------------------------------------------------
			0x20a// ECL 1 (4 factors) -------------------------------------------------------------------------------
            null,
            0x238,
            0x2d3,
            0x329
        ), //   4
		array// 4 // ECL 2 (8 factors) -------------------------------------------------------------------------------
			0x0ed// ECL 2 (8 factors) -------------------------------------------------------------------------------
            null,
            0x134,
            0x1b4,
            0x11c,
		...
		array// 8 // ECL 3 (16 factors) ------------------------------------------------------------------------------
			0x112// ECL 3 (16 factors) ------------------------------------------------------------------------------
            null,
            0x232,
            0x0e8,
            0x2f3,
		...
		array// 16 // ECL 4 (32 factors) ------------------------------------------------------------------------------
			0x169// ECL 4 (32 factors) ------------------------------------------------------------------------------
            null,
            0x23f,
            0x39a,
            0x20d,
		...
		array// 32 // ECL 5 (64 factors) ------------------------------------------------------------------------------
			0x21b// ECL 5 (64 factors) ------------------------------------------------------------------------------
            null,
            0x1a6,
            0x006,
            0x05d,
		...
		array// 64 // ECL 6 (128 factors) -----------------------------------------------------------------------------
			0x209// ECL 6 (128 factors) -----------------------------------------------------------------------------
            null,
            0x136,
            0x360,
            0x223,
		...
		array// 128 // ECL 7 (256 factors) -----------------------------------------------------------------------------
			0x20c// ECL 7 (256 factors) -----------------------------------------------------------------------------
            null,
            0x37e,
            0x04b,
            0x2fe,
		...
		array// 256 // ECL 8 (512 factors) -----------------------------------------------------------------------------
			0x160// ECL 8 (512 factors) -----------------------------------------------------------------------------
            null,
            0x04d,
            0x175,
            0x1f8,
    ...
    public function __construct($code, $ecl = -1, $aspectratio = 2, $macro = array())
    {
		...
		$sequence// get the input sequence array
        null = $this->getInputSequences($code);
        $codewords = array(); // array of code-words
		foreach// array of code-words
        null ($sequence as $seq) {
        ...
        if ($codewords[0] == 900) {
			...
			// Text Alpha is the default mode, so remove the first code
            nullarray_shift($codewords);
        }
        // count number of codewords
		$numcw// count number of codewords
        null = count($codewords);
        if ($numcw > 925) {
            // reached maximum data codeword capacity
			return// reached maximum data codeword capacity
            null false;
        }
        // build macro control block codewords
		if// build macro control block codewords
        null (! empty($macro)) {
            $macrocw = array();
            // beginning of macro control block
			$macrocw// beginning of macro control block
            null[] = 928;
            // segment index
			$cw// segment index
            null = $this->getCompaction(902, sprintf('%05d', $macro['segment_index']), false);
            $macrocw = array_merge($macrocw, $cw);
            // file ID
			$cw// file ID
            null = $this->getCompaction(900, $macro['file_id'], false);
            $macrocw = array_merge($macrocw, $cw);
            // optional fields
			$optmodes// optional fields
            null = array(
                900,
                902,
                902,
            ...
            if ($macro['segment_index'] == ($macro['segment_total'] - 1)) {
				...
				$macrocw// end of control block
                null[] = 922;
            }
            // update total codewords
			$numcw// update total codewords
            null += count($macrocw);
        }
        // set error correction level
		$ecl// set error correction level
        null = $this->getErrorCorrectionLevel($ecl, $numcw);
        // number of codewords for error correction
		$errsize// number of codewords for error correction
        null = (2 << $ecl);
        // calculate number of columns (number of codewords per row) and rows
		$nce// calculate number of columns (number of codewords per row) and rows
        null = ($numcw + $errsize + 1);
        ...
        if ($size > 928) {
			...
			if// set dimensions to get maximum capacity
            null (abs($aspectratio - (17 * 29 / 32)) < abs($aspectratio - (17 * 16 / 58))) {
                $cols = 29;
                $rows = 32;
            } else {
                $cols = 16;
        ...
        if ($pad > 0) {
            if (($size - $rows) == $nce) {
                ...
                // add pading
				$codewords// add pading
                null = array_merge($codewords, array_fill(0, $pad, 900));
            }
        }
        if (! empty($macro)) {
            // add macro section
			$codewords// add macro section
            null = array_merge($codewords, $macrocw);
        }
        // Symbol Lenght Descriptor (number of data codewords including Symbol Lenght Descriptor and pad codewords)
		$sld// Symbol Lenght Descriptor (number of data codewords including Symbol Lenght Descriptor and pad codewords)
        null = $size - $errsize;
        // add symbol length description
		// add symbol length description
        nullarray_unshift($codewords, $sld);
        // calculate error correction
		$ecw// calculate error correction
        null = $this->getErrorCorrection($codewords, $ecl);
        // add error correction codewords
		$codewords// add error correction codewords
        null = array_merge($codewords, $ecw);
        // add horizontal quiet zones to start and stop patterns
		$pstart// add horizontal quiet zones to start and stop patterns
        null = str_repeat('0', QUIETH) . $this->start_pattern;
		...
		if// build rows for vertical quiet zone
        null (QUIETV > 0) {
            $empty_row = array_fill(0, $barcode_array['num_cols'], 0);
            for ($i = 0; $i < QUIETV; ++ $i) {
                // add vertical quiet rows
				$barcode_array// add vertical quiet rows
                null['bcode'][] = $empty_row;
            }
        }
        $k = 0; // codeword index
		$cid// codeword index
        null = 0; // initial cluster
                  // for each row
		for// for each row
        null ($r = 0; $r < $rows; ++ $r) {
            // row start code
            ...
            // left row indicator
			$row// left row indicator
            null .= sprintf('%17b', $this->clusters[$cid][$L]);
            // for each column
			for// for each column
            null ($c = 0; $c < $cols; ++ $c) {
            ...
            // right row indicator
			$row// right row indicator
            null .= sprintf('%17b', $this->clusters[$cid][$L]);
            // row stop code
			$row// row stop code
            null .= $pstop;
            // convert the string to array
			$arow// convert the string to array
            null = preg_split('//', $row, - 1, PREG_SPLIT_NO_EMPTY);
            // duplicate row to get the desired height
			for// duplicate row to get the desired height
            null ($h = 0; $h < ROWHEIGHT; ++ $h) {
    ...
    protected function getErrorCorrectionLevel($ecl, $numcw)
    {
		...
		$maxecl// get maximum correction level
        null = 8; // starting error level
		$maxerrsize// starting error level
        null = (928 - $numcw); // available codewords for error
		while// available codewords for error
        null ($maxecl > 0) {
            $errsize = (2 << $ecl);
    ...
    protected function getErrorCorrection($cw, $ecl)
    {
		...
		$ecc// get error correction coefficients
        null = $this->rsfactors[$ecl];
        // number of error correction factors
		$eclsize// number of error correction factors
        null = (2 << $ecl);
        // maximum index for $rsfactors[$ecl]
		$eclmaxid// maximum index for $rsfactors[$ecl]
        null = ($eclsize - 1);
        // initialize array of error correction codewords
		$ecw// initialize array of error correction codewords
        null = array_fill(0, $eclsize, 0);
        // for each data codeword
		foreach// for each data codeword
        null ($cw as $k => $d) {
    ...
    protected function getInputSequences($code)
    {
		...
		$numseq// array to be returned
        null = array();
        // get numeric sequences
		// get numeric sequences
        nullpreg_match_all('/([0-9]{13,})/', $code, $numseq, PREG_OFFSET_CAPTURE);
        $numseq[1][] = array(
            '',
            strlen($code)
        );
        ...
        foreach ($numseq[1] as $seq) {
            ...
            if ($seq[1] > 0) {
				...
				$prevseq// extract text sequence before the number sequence
                null = substr($code, $offset, ($seq[1] - $offset));
                $textseq = array();
                // get text sequences
				// get text sequences
                nullpreg_match_all('/([\x09\x0a\x0d\x20-\x7e]{5,})/', $prevseq, $textseq, PREG_OFFSET_CAPTURE);
                $textseq[1][] = array(
                    '',
                    strlen($prevseq)
                );
                ...
                foreach ($textseq[1] as $txtseq) {
                    ...
                    if ($txtseq[1] > 0) {
						...
						$prevtxtseq// extract byte sequence before the text sequence
                        null = substr($prevseq, $txtoffset, ($txtseq[1] - $txtoffset));
                        if (strlen($prevtxtseq) > 0) {
                            // add BYTE sequence
							if// add BYTE sequence
                            null ((strlen($prevtxtseq) == 1) and ((count($sequence_array) > 0) and ($sequence_array[(count($sequence_array) - 1)][0] == 900))) {
                    ...
                    if ($txtseqlen > 0) {
						...
						$sequence_array// add numeric sequence
                        null[] = array(
                            900,
                            $txtseq[0]
                        );
    ...
    protected function getCompaction($mode, $code, $addmode = true)
    {
		...
		switch// array of codewords to return
        null ($mode) {
            case 900:
                { // Text Compaction mode latch
				$submode// Text Compaction mode latch
                    null = 0; // default Alpha sub-mode
				$txtarr// default Alpha sub-mode
                    null = array(); // array of characters and sub-mode switching characters
				$codelen// array of characters and sub-mode switching characters
                    null = strlen($code);
                    for ($i = 0; $i < $codelen; ++ $i) {
                        $chval = ord($code{$i});
                        if (($k = array_search($chval, $this->textsubmodes[$submode])) !== false) {
                            // we are on the same sub-mode
						$txtarr// we are on the same sub-mode
                            null[] = $k;
                        } else {
                            // the sub-mode is changed
						for// the sub-mode is changed
                            null ($s = 0; $s < 4; ++ $s) {
                                // search new sub-mode
							if// search new sub-mode
                                null (($s != $submode) and (($k = array_search($chval, $this->textsubmodes[$s])) !== false)) {
                                    // $s is the new submode
								if// $s is the new submode
                                    null (((($i + 1) == $codelen) or ((($i + 1) < $codelen) and (array_search(ord($code{($i + 1)}), $this->textsubmodes[$submode]) !== false))) and (($s == 3) or (($s == 0) and ($submode == 1)))) {
                                        // shift (temporary change only for this char)
									if// shift (temporary change only for this char)
                                        null ($s == 3) {
                                            // shift to puntuaction
										$txtarr// shift to puntuaction
                                            null[] = 29;
                                        } else {
                                            // shift from lower to alpha
										$txtarr// shift from lower to alpha
                                            null[] = 27;
                                        }
                                    } else {
                                        // latch
									$txtarr// latch
                                        null = array_merge($txtarr, $this->textlatch['' . $submode . $s]);
                                        // set new submode
									$submode// set new submode
                                        null = $s;
                                    }
                                    // add characted code to array
								$txtarr// add characted code to array
                                    null[] = $k;
                                    break;
                                }
                            }
                    ...
                    // calculate codewords
				for// calculate codewords
                    null ($i = 0; $i < $txtarrlen; $i += 2) {
            ...
            case 924:
				...
				while// Byte Compaction mode latch
                    null (($codelen = strlen($code)) > 0) {
                        if ($codelen > 6) {
                            $rest = substr($code, 6);
            ...
            case 902:
				...
				while// Numeric Compaction mode latch
                    null (($codelen = strlen($code)) > 0) {
                        if ($codelen > 44) {
                            $rest = substr($code, 44);
