...
$objFunc->SetTitle("Mantenimiento de Tablas Auxiliares - Modulo Administración");
$objFunc->SetSubTitle("Mantenimiento de Tablas Auxiliares");
...
<script type="text/javascript">
	...
	function LoadDataGrid()
	{
		...
		var xmlurl = "man.xml.php?action=<?php echo(md5("lista_tablas_aux"));?>&idTabla="+idtabla ;
		...
		{ 
			dsLista = new Spry.Data.XMLDataSet(xmlurl, "tipos/rowdata", {useCache: false});
...
<body class="oneColElsCtrHdr">
    ...
    $objFunc->llenarCombo($rs, 'cod_tabla', 'nom_tabla', '', 'cod_tabla');
    ?>
    </select></td>
					</tr>
					<tr>
					...
					<div class="TableGrid" spry:region="pvLista">
						<table class="grid-table grid-width">
							<thead>
								<tr>
									<th width="44">&nbsp;</th>
									<th width="50" height="25" align="center" spry:sort="codigo">Codigo</th>
 ...
 function btnNuevo_Clic()
  {
	...
	$("#divContent").css('display', 'none');
	var url = "man_tipos_edit.php?mode=<?php echo(md5("ajax_new"));?>&id=&idtabla="+$('#cbotabla').val() ;
	loadUrlSpry("divContentEdit",url);
	return;

  }
 function btnEditar_Clic(idtablaaux)
  {
	...
	var url = "man_tipos_edit.php?mode=<?php echo(md5("ajax_edit"));?>&id="+idtablaaux+"&idtabla="+$('#cbotabla').val() ;
	loadUrlSpry("divContentEdit",url);
	$("#divContent").fadeOut("slow");
	$("#divContent").css('display', 'none');
 ...
 function CancelEdit()
 {
  ...
  function Eliminar(codigo,Descripcion)
  {
	...
	if(confirm("Estas seguro de eliminar el Registro \n" + Descripcion)) {
		if(confirm("Tenga en cuenta que si existen otros registros que usan este item,\n estos también seran eliminados. \nEsta realmente seguro de eliminar: \n" + Descripcion+" ?")) {
			var BodyForm = "idTipo="+codigo;
			var sURL = "man_tipos_process.php?action=<?php echo(md5("ajax_del"))?>";
