...
?>
  ...
  Directorio Actual: <?php echo($VirtualPath);?>
	...
	{
		var path = "<?php echo($VirtualPath);?>";
		var pagina = path + file ;
		var arrParams = new Array();
		arrParams[0] = "mnu_cod=" + $("#txtcodigo").val();
		arrParams[1] = "pag_nomb=" + file ;
		arrParams[2] = "pag_link=" + pagina;
		
		var BodyForm = arrParams.join("&");
	 	var sURL = "man_mnu_process.php?action=<?php echo(md5("ajax_agrega_menu_paginas"));?>" ;
	    var req = Spry.Utils.loadURL("POST", sURL, true, SuccessAgregarPaginaMenu, { postData: BodyForm, headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8" }, errorCallback: MyErrorCallback });
		return false ;
	}
	function AbrirDirectorio(dir)
	{
		var querystring = "<?php echo($objFunc->__QueryString("dir"));?>&dir=<?php echo($path)?>"+dir;
		var url = 'man_mnu_pag_sec.php' + querystring ;
		loadPopup("Agregar Pagina Secundaria de [" + $('#txtnombre').val() + "]" ,url)
		return false ;
	}
	
	
	function SuccessAgregarPaginaMenu(req)
	{
